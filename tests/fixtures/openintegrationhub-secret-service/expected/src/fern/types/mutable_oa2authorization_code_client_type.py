...
class MutableOa2AuthorizationCodeClientType(enum.StrEnum):
    ...
    def visit(
        ...
        if self is MutableOa2AuthorizationCodeClientType.OA1TWO_LEGGED:
            return oa1two_legged()
        if self is MutableOa2AuthorizationCodeClientType.OA1THREE_LEGGED:
            return oa1three_legged()
        if self is MutableOa2AuthorizationCodeClientType.OA2AUTHORIZATION_CODE:
            return oa2authorization_code()
        if self is MutableOa2AuthorizationCodeClientType.SESSION_AUTH:
            return session_auth()
