...
class FolderQuotaUpdateUsageRequestMode(enum.StrEnum):
    """
    Update type:
    ...
    def visit(self, add: typing.Callable[[], T_Result], reset: typing.Callable[[], T_Result]) -> T_Result:
        if self is FolderQuotaUpdateUsageRequestMode.ADD:
            return add()
        if self is FolderQuotaUpdateUsageRequestMode.RESET:
            return reset()
