...
class PatchViewViewType(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is PatchViewViewType.PROJECTS:
            return projects()
        if self is PatchViewViewType.EXPERIMENTS:
            return experiments()
        if self is PatchViewViewType.EXPERIMENT:
            return experiment()
        if self is PatchViewViewType.PLAYGROUNDS:
            return playgrounds()
        if self is PatchViewViewType.PLAYGROUND:
            return playground()
        if self is PatchViewViewType.DATASETS:
            return datasets()
        if self is PatchViewViewType.DATASET:
            return dataset()
        if self is PatchViewViewType.PROMPTS:
            return prompts()
        if self is PatchViewViewType.PARAMETERS:
            return parameters()
        if self is PatchViewViewType.TOOLS:
            return tools()
        if self is PatchViewViewType.SCORERS:
            return scorers()
        if self is PatchViewViewType.CLASSIFIERS:
            return classifiers()
        if self is PatchViewViewType.LOGS:
            return logs()
        if self is PatchViewViewType.MONITOR:
            return monitor()
        if self is PatchViewViewType.FOR_REVIEW_PROJECT_LOG:
            return for_review_project_log()
        if self is PatchViewViewType.FOR_REVIEW_EXPERIMENTS:
            return for_review_experiments()
        if self is PatchViewViewType.FOR_REVIEW_DATASETS:
