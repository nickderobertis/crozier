...
class GetOrdersResponseShippingAddressJapanType(enum.StrEnum):
    """
    ...
    def visit(self, kana: typing.Callable[[], T_Result], kanji: typing.Callable[[], T_Result]) -> T_Result:
        if self is GetOrdersResponseShippingAddressJapanType.KANA:
            return kana()
        if self is GetOrdersResponseShippingAddressJapanType.KANJI:
            return kanji()
