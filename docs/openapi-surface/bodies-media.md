...
`*/*` and `type/*` ranges, a key carrying an RFC 7231 `; parameter=value`, and a
key that is none of those), plus the five named types a generator has to
dispatch on — `multipart/*`,
...
| media-type-key-parameters | both | Media Type Object content-map key | golden | Census object-model map-key walk for a key carrying an RFC 7231 `; parameter=value`: 33 declarations in `openbanking.org.uk-account-info-openapi` (32, every one `application/json; charset=utf-8`) and `anchore.io` (1, `text/plain; utf-8`). Crozier's media dispatch reads these keys literally, so neither is the `application/json` that `response_schema` and `selected_json_request_media` look up by name. | | | |
...
The limitations join is the `grep`
[`../openapi-surface-coverage.md`](../openapi-surface-coverage.md) prescribes.
Twelve of the keys it returns are this region's and are joined above;
`status_code` is not one of them — it is a row label inside the ledger's 407/421
probe table rather than a feature key with a verdict, so no row here joins on
it.
