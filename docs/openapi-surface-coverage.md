...
`tests/fixtures/<name>/openapi.*` documents, and the 147 `link-ok` documents
...
- **`just surface-census`**, for the classifications and for criterion 4. The
  ...
  It reads **179** registered sources, of which
  **162** carry a committed golden. `document-paths`'s own evidence cells are
...
The witness searches' registrations add seven sources — corpus rows 144 to 150,
`paloalto-cspm-alerts`, `paloalto-cspm-reports`, `paloalto-cspm-search-manager`,
`thrivecart`, `truefoundry-trueforge-5adde28`, `fergus` and `groupe-psa` (32
vendored plus 147 `link-ok`) — and promote five `schemas` features to `golden`:
...
way; twenty-four of them are still declared by no *golden-bearing* registered
source, which is what a `gap` is, and the other five are `golden` now.
