...
**What it says today.** The walk enumerates 530 features. 417 are `golden`: a
...
45 are `gap`: 20 of them
`UNREACHABLE` — the shape has no position in a generated Python SDK at all — and
25 `FIXTURE`, each a shape a real document can declare and no committed golden
reaches. So *does crozier byte-match Fern on
every OpenAPI feature and scenario?* **No.** The honest answer is that byte-match
evidence covers 417 of the 530 features this walk can see, that 68 more carry a
Fern verdict and no byte comparison at all, that 45 have neither, and that the
...
`tests/fixtures/<name>/openapi.*` documents, and the 151 `link-ok` documents
...
- **`just surface-census`**, for the classifications and for criterion 4. The
  ...
  It reads **183** registered sources, of which
  **166** carry a committed golden. `document-paths`'s own evidence cells are
...
| [`schemas`](openapi-surface/schemas.md) | 240 | 193 | 21 | 26 | 23 | 0 | 3 |
...
| **total** | **530** | **417** | **68** | **45** | **25** | **0** | **20** |

The walk enumerated **530** features and landed each in exactly one category:
**417** `golden`, **68** `limitations`, **45** `gap`. The `gap` column splits by
settlement class into **25** `FIXTURE`, **0** `PROBE` and **20** `UNREACHABLE`.
...
The witness searches' registrations add eleven sources — corpus rows 144 to
154, `paloalto-cspm-alerts`, `paloalto-cspm-reports`,
`paloalto-cspm-search-manager`, `thrivecart`, `truefoundry-trueforge-5adde28`,
`fergus`, `groupe-psa`, `timelyapp`, `nextgen`, `auto-agent-protocol` and `skool`
(32 vendored plus 151 `link-ok`) — and promote seven `schemas` features to `golden`:
...
**What the `golden` count means, and what it does not.** 417 of those 530
...
real bytes there and `just check` fails if they diverge. The other 113 do not. 68
carry a Fern verdict measured on a probe and no byte comparison at all, and 45
have neither. Neither
column is a defect count, and neither 417 nor 530 is a claim of exhaustion —
...
**What the `gap` count means.** 45 is the number of OpenAPI shapes for which
...
nothing about what Fern emits. `just check` is green over all 45
...
That leaves 25 for the two backlogs below, every one of them in the fixture one.
...
is a screened real-world witness rather than a locally authored probe. Seven
...
twenty-five rows. It carried thirty-two before corpus rows 144 to 153 registered
witnesses for seven of them, and thirty of those thirty-two never stood here
...
way; twenty-three of them are still declared by no *golden-bearing* registered
source, which is what a `gap` is, and the other six are `golden` now.
...
- **Criterion 4**, witness supply: registered sources the census reports
  ...
  is what makes it a gap — 25 of the 25 score zero here, no registered source
...
**The median blind-spot count of this list is 235** — twenty-three entries name
`src/ir.rs` and nothing else, `securityscheme-ref` names `src/openapi.rs` alone and
`oauth2-password` names both, and 0 of the 25 entries name no `src/` file at all,
which is the mirror image of the list this one replaced. 0 of the 25 ranked
entries reach no blind region. Criterion 4 separates none of them, and among the
twenty-three rows that name one `src/ir.rs` place, criterion 5 decides. Criterion 1 puts the two rows naming more than one place
...
| 6 | [`array-item-inheritance-union`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 7 | [`array-item-pointer-walk-anyof`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 8 | [`array-item-pointer-walk-oneof`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 9 | [`oneof-array-variant-annotated-ref-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 10 | [`oneof-array-variant-anyof-discriminated-union-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
...
| 14 | [`oneof-array-variant-composed-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 15 | [`oneof-array-variant-empty-object-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 16 | [`oneof-bare-object-example-variant`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 17 | [`property-sole-anyof-closed-object-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 18 | [`property-sole-anyof-composed-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 19 | [`property-sole-anyof-empty-object-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 20 | [`property-sole-anyof-struct-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
...
| 25 | [`securityscheme-ref`](openapi-surface/security.md) | `security` | **3** (`src/openapi.rs` 3) | **218** (`src/openapi.rs` 218) | **2** (client.py, core/) | **0** |
