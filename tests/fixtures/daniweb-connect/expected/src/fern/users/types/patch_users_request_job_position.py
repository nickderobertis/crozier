...
class PatchUsersRequestJobPosition(enum.StrEnum):
    ...
    def visit(
        ...
        if self is PatchUsersRequestJobPosition.EXECUTIVE_MANAGEMENT_C_LEVEL:
            return executive_management_c_level()
        if self is PatchUsersRequestJobPosition.VP_LEVEL_EXECUTIVE:
            return vp_level_executive()
        if self is PatchUsersRequestJobPosition.MANAGER_DIRECTOR_SUPERVISOR:
            return manager_director_supervisor()
        if self is PatchUsersRequestJobPosition.SYSTEMS_DEVELOPMENT:
            return systems_development()
        if self is PatchUsersRequestJobPosition.SOFTWARE_DEVELOPMENT:
            return software_development()
        if self is PatchUsersRequestJobPosition.WEB_DEVELOPER:
            return web_developer()
        if self is PatchUsersRequestJobPosition.IT_CONSULTANT:
            return it_consultant()
        if self is PatchUsersRequestJobPosition.TECHNICAL_SUPPORT:
            return technical_support()
        if self is PatchUsersRequestJobPosition.SALES:
            return sales()
        if self is PatchUsersRequestJobPosition.OTHER_TECHNOLOGY_RELATED:
            return other_technology_related()
        if self is PatchUsersRequestJobPosition.OTHER_NON_TECHNOLOGY_RELATED:
            return other_non_technology_related()
        if self is PatchUsersRequestJobPosition.STUDENT:
            return student()
        if self is PatchUsersRequestJobPosition.RETIRED:
            return retired()
