...
class ParticipantAttributesNpqEnrolmentsItemWithdrawalReason(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.INSUFFICIENT_CAPACITY_TO_UNDERTAKE_PROGRAMME:
            return insufficient_capacity_to_undertake_programme()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.PERSONAL_REASON_HEALTH_OR_PREGNANCY_RELATED:
            return personal_reason_health_or_pregnancy_related()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.PERSONAL_REASON_MOVING_SCHOOL:
            return personal_reason_moving_school()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.PERSONAL_REASON_OTHER:
            return personal_reason_other()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.INSUFFICIENT_CAPACITY:
            return insufficient_capacity()
        if (
            self
            is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.CHANGE_IN_DEVELOPMENTAL_OR_PERSONAL_PRIORITIES
        ):
            return change_in_developmental_or_personal_priorities()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.CHANGE_IN_SCHOOL_CIRCUMSTANCES:
            return change_in_school_circumstances()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.CHANGE_IN_SCHOOL_LEADERSHIP:
            return change_in_school_leadership()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.QUALITY_OF_PROGRAMME_STRUCTURE_NOT_SUITABLE:
            return quality_of_programme_structure_not_suitable()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.QUALITY_OF_PROGRAMME_CONTENT_NOT_SUITABLE:
            return quality_of_programme_content_not_suitable()
        if (
            self
            is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.QUALITY_OF_PROGRAMME_FACILITATION_NOT_EFFECTIVE
        ):
            return quality_of_programme_facilitation_not_effective()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.QUALITY_OF_PROGRAMME_ACCESSIBILITY:
            return quality_of_programme_accessibility()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.QUALITY_OF_PROGRAMME_OTHER:
            return quality_of_programme_other()
        if (
            self
            is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.PROGRAMME_NOT_APPROPRIATE_FOR_ROLE_AND_CPD_NEEDS
        ):
            return programme_not_appropriate_for_role_and_cpd_needs()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.STARTED_IN_ERROR:
            return started_in_error()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.EXPECTED_COMMITMENT_UNCLEAR:
            return expected_commitment_unclear()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.ASSESSMENT_REQUIREMENTS_NOT_MET:
            return assessment_requirements_not_met()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.CHANGE_IN_CAREER:
            return change_in_career()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.DISENGAGED_AND_UNRESPONSIVE:
            return disengaged_and_unresponsive()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.NON_PAYMENT_OF_INVOICE:
            return non_payment_of_invoice()
        if self is ParticipantAttributesNpqEnrolmentsItemWithdrawalReason.OTHER:
            return other()
