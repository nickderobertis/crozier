...
class GetOrdersResponseDisputeLastStatus(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is GetOrdersResponseDisputeLastStatus.WARNING_NEEDS_RESPONSE:
            return warning_needs_response()
        if self is GetOrdersResponseDisputeLastStatus.WARNING_UNDER_REVIEW:
            return warning_under_review()
        if self is GetOrdersResponseDisputeLastStatus.WARNING_CLOSED:
            return warning_closed()
        if self is GetOrdersResponseDisputeLastStatus.NEEDS_RESPONSE:
            return needs_response()
        if self is GetOrdersResponseDisputeLastStatus.UNDER_REVIEW:
            return under_review()
        if self is GetOrdersResponseDisputeLastStatus.CHARGE_REFUNDED:
            return charge_refunded()
        if self is GetOrdersResponseDisputeLastStatus.WON:
            return won()
        if self is GetOrdersResponseDisputeLastStatus.LOST:
            return lost()
