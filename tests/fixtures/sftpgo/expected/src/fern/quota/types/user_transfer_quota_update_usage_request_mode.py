...
class UserTransferQuotaUpdateUsageRequestMode(enum.StrEnum):
    """
    Update type:
    ...
    def visit(self, add: typing.Callable[[], T_Result], reset: typing.Callable[[], T_Result]) -> T_Result:
        if self is UserTransferQuotaUpdateUsageRequestMode.ADD:
            return add()
        if self is UserTransferQuotaUpdateUsageRequestMode.RESET:
            return reset()
