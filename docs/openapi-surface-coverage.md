...
**What it says today.** The walk enumerates 530 features. 414 are `golden`: a
...
48 are `gap`: 20 of them
`UNREACHABLE` — the shape has no position in a generated Python SDK at all — and
28 `FIXTURE`, each a shape a real document can declare and no committed golden
reaches. So *does crozier byte-match Fern on
every OpenAPI feature and scenario?* **No.** The honest answer is that byte-match
evidence covers 414 of the 530 features this walk can see, that 68 more carry a
Fern verdict and no byte comparison at all, that 48 have neither, and that the
...
- **A case carries a selector only when that selector is exact; otherwise it is a
  ...
  `hoist_union_variant` sends all three to `base_type_ref`. Ninety-five of the
  ninety-five cases below survive this test; the other zero name the
...
The conjunctions are themselves a closed list of 82, declared in
...
| [`schemas`](openapi-surface/schemas.md) | 240 | 190 | 21 | 29 | 26 | 0 | 3 |
...
| **total** | **530** | **414** | **68** | **48** | **28** | **0** | **20** |

The walk enumerated **530** features and landed each in exactly one category:
**414** `golden`, **68** `limitations`, **48** `gap`. The `gap` column splits by
settlement class into **28** `FIXTURE`, **0** `PROBE` and **20** `UNREACHABLE`.
...
The witness searches' registrations add five sources — corpus rows 144 to 148,
`paloalto-cspm-alerts`, `paloalto-cspm-reports`, `paloalto-cspm-search-manager`,
`thrivecart` and `truefoundry-trueforge-5adde28` (32 vendored plus 145
`link-ok`) — and promote four `schemas` features to `golden`:
`annotated-ref-target-composed`, `annotated-ref-target-oneof`,
`annotated-ref-target-closed-object` and `ref-pointer-undeclared-component-head`.
The denominator is unchanged. None of the four goldens reaches the `src/ir.rs`
site its row names; each row's evidence cell says what the golden reaches instead.

**What the `golden` count means, and what it does not.** 414 of those 530
...
real bytes there and `just check` fails if they diverge. The other 116 do not. 68
carry a Fern verdict measured on a probe and no byte comparison at all, and 48
have neither. Neither
column is a defect count, and neither 414 nor 530 is a claim of exhaustion —
...
**What the `gap` count means.** 48 is the number of OpenAPI shapes for which
...
nothing about what Fern emits. `just check` is green over all 48
...
That leaves 28 for the two backlogs below, every one of them in the fixture one.
...
is a screened real-world witness rather than a locally authored probe. Four
...
**Each feature is classified exactly once.** The 530 rows carry 530 distinct
...
branches those six functions of `src/ir.rs` offer a document, all 95 carry an exact
...
twenty-eight rows. It carried thirty-two before corpus rows 144 to 148 registered
witnesses for four of them, and thirty of those thirty-two never stood here
...
way; twenty-five of them are still declared by no *golden-bearing* registered
source, which is what a `gap` is, and the other four are `golden` now.
...
- **Criterion 4**, witness supply: registered sources the census reports
  ...
  is what makes it a gap — 28 of the 28 score zero here, no registered source
...
**The median blind-spot count of this list is 235** — twenty-five entries name
...
`oauth2-password` names both, and 0 of the 28 entries name no `src/` file at all,
which is the mirror image of the list this one replaced. 0 of the 28 ranked
entries reach no blind region. Criterion 4 separates none of them, and among the
twenty-four rows that name one `src/ir.rs` place, criterion 5 decides. Criterion 1 puts the three rows naming more than one place
...
|---|---|---|---|---|---|---|
| 1 | [`annotated-ref-target-string-const`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 2 | [`anyof-array-variant-anyof-nullable-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 3 | [`anyof-array-variant-closed-object-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 4 | [`anyof-array-variant-empty-object-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 5 | [`anyof-array-variant-oneof-nullable-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 6 | [`anyof-array-variant-struct-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 7 | [`array-item-inheritance-union`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 8 | [`array-item-pointer-walk-anyof`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 9 | [`array-item-pointer-walk-oneof`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 10 | [`oneof-array-variant-annotated-ref-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 11 | [`oneof-array-variant-anyof-discriminated-union-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 12 | [`oneof-array-variant-anyof-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 13 | [`oneof-array-variant-anyof-nullable-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 14 | [`oneof-array-variant-closed-object-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 15 | [`oneof-array-variant-composed-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 16 | [`oneof-array-variant-empty-object-item`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 17 | [`oneof-bare-object-example-variant`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 18 | [`property-sole-anyof-closed-object-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 19 | [`property-sole-anyof-composed-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 20 | [`property-sole-anyof-empty-object-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 21 | [`property-sole-anyof-struct-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 22 | [`property-sole-oneof-closed-object-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 23 | [`property-sole-oneof-composed-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 24 | [`property-sole-oneof-empty-object-member`](openapi-surface/schemas.md) | `schemas` | **1** (`src/ir.rs` 1) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 25 | [`schema-name-nonidentifier-name`](openapi-surface/schemas.md) | `schemas` | **1** (`src/naming.rs` 1) | **31** (`src/naming.rs` 31) | **1** (types/) | **0** |
| 26 | [`oauth2-password`](openapi-surface/security.md) | `security` | **2** (`src/ir.rs` 1, `src/openapi.rs` 1) | **453** (`src/ir.rs` 235, `src/openapi.rs` 218) | **3** (types/, client.py, core/) | **0** |
| 27 | [`ref-pointer-unnamed-segment`](openapi-surface/schemas.md) | `schemas` | **2** (`src/ir.rs` 2) | **235** (`src/ir.rs` 235) | **1** (types/) | **0** |
| 28 | [`securityscheme-ref`](openapi-surface/security.md) | `security` | **3** (`src/openapi.rs` 3) | **218** (`src/openapi.rs` 218) | **2** (client.py, core/) | **0** |
