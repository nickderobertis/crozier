...
class UpdateSettingsRequestResolvedTopicNoticeAutoReadPolicy(enum.StrEnum):
    """
    ...
    **Changes**: New in Zulip 11.0 (feature level 385).
    ...
    def visit(
        ...
        if self is UpdateSettingsRequestResolvedTopicNoticeAutoReadPolicy.ALWAYS:
            return always()
        if self is UpdateSettingsRequestResolvedTopicNoticeAutoReadPolicy.EXCEPT_FOLLOWED:
            return except_followed()
        if self is UpdateSettingsRequestResolvedTopicNoticeAutoReadPolicy.NEVER:
            return never()
