...
class TopPagesReportsResponseSortBy(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is TopPagesReportsResponseSortBy.SESSION:
            return session()
        if self is TopPagesReportsResponseSortBy.USER:
            return user()
        if self is TopPagesReportsResponseSortBy.PAGEVIEW:
            return pageview()
