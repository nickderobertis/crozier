...
class DocVersionsSignatureContentsResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocVersionsSignatureContentsResponseCode.UNKNOWN:
            return unknown()
        if self is DocVersionsSignatureContentsResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocVersionsSignatureContentsResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocVersionsSignatureContentsResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocVersionsSignatureContentsResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocVersionsSignatureContentsResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocVersionsSignatureContentsResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocVersionsSignatureContentsResponseCode.ABORTED:
            return aborted()
        if self is DocVersionsSignatureContentsResponseCode.NETWORK:
            return network()
        if self is DocVersionsSignatureContentsResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocVersionsSignatureContentsResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocVersionsSignatureContentsResponseCode.NOT_FOUND:
            return not_found()
        if self is DocVersionsSignatureContentsResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocVersionsSignatureContentsResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocVersionsSignatureContentsResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocVersionsSignatureContentsResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocVersionsSignatureContentsResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocVersionsSignatureContentsResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocVersionsSignatureContentsResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocVersionsSignatureContentsResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocVersionsSignatureContentsResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocVersionsSignatureContentsResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocVersionsSignatureContentsResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocVersionsSignatureContentsResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocVersionsSignatureContentsResponseCode.STALE_BASE:
            return stale_base()
