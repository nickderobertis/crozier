...
#
# llmlint: ignore-file[robust_shell, tool_output_is_signal] deliberate for a
# session-startup installer: `set -e` is omitted so a flaky step can't abort the
# hook — the script owns its exit codes and always exits 0; success stays quiet and
