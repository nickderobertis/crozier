...
class EngineErrorPayloadCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is EngineErrorPayloadCode.UNKNOWN:
            return unknown()
        if self is EngineErrorPayloadCode.INVALID_ARG:
            return invalid_arg()
        if self is EngineErrorPayloadCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is EngineErrorPayloadCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is EngineErrorPayloadCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is EngineErrorPayloadCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is EngineErrorPayloadCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is EngineErrorPayloadCode.ABORTED:
            return aborted()
        if self is EngineErrorPayloadCode.NETWORK:
            return network()
        if self is EngineErrorPayloadCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is EngineErrorPayloadCode.FORBIDDEN:
            return forbidden()
        if self is EngineErrorPayloadCode.NOT_FOUND:
            return not_found()
        if self is EngineErrorPayloadCode.WIRE_FORMAT:
            return wire_format()
        if self is EngineErrorPayloadCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is EngineErrorPayloadCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is EngineErrorPayloadCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is EngineErrorPayloadCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is EngineErrorPayloadCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is EngineErrorPayloadCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is EngineErrorPayloadCode.SIGNING_PENDING:
            return signing_pending()
        if self is EngineErrorPayloadCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is EngineErrorPayloadCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is EngineErrorPayloadCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is EngineErrorPayloadCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is EngineErrorPayloadCode.STALE_BASE:
            return stale_base()
