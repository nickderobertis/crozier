...
class LambdaFunctionAssociationEventType(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is LambdaFunctionAssociationEventType.VIEWER_REQUEST:
            return viewer_request()
        if self is LambdaFunctionAssociationEventType.VIEWER_RESPONSE:
            return viewer_response()
        if self is LambdaFunctionAssociationEventType.ORIGIN_REQUEST:
            return origin_request()
        if self is LambdaFunctionAssociationEventType.ORIGIN_RESPONSE:
