...
class ItemStatus(enum.StrEnum):
    ...
    def visit(
        ...
        if self is ItemStatus.TAXCLOUD_INVITED:
            return taxcloud_invited()
        if self is ItemStatus.TAXCLOUD_CONNECTED:
            return taxcloud_connected()
        if self is ItemStatus.TAXCLOUD_DISCONNECTED:
            return taxcloud_disconnected()
        if self is ItemStatus.EXTERNAL_COMPLIANCE:
