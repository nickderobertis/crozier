...
class MutableSessionAuthClientType(enum.StrEnum):
    ...
    def visit(
        ...
        if self is MutableSessionAuthClientType.OA1TWO_LEGGED:
            return oa1two_legged()
        if self is MutableSessionAuthClientType.OA1THREE_LEGGED:
            return oa1three_legged()
        if self is MutableSessionAuthClientType.OA2AUTHORIZATION_CODE:
            return oa2authorization_code()
        if self is MutableSessionAuthClientType.SESSION_AUTH:
            return session_auth()
