...
class PatchUsersRequestCompanySize(enum.StrEnum):
    ...
    def visit(
        ...
        if self is PatchUsersRequestCompanySize.SELF_EMPLOYED:
            return self_employed()
        if self is PatchUsersRequestCompanySize.TWO9EMPLOYEES:
            return two9employees()
        if self is PatchUsersRequestCompanySize.TEN49EMPLOYEES:
            return ten49employees()
        if self is PatchUsersRequestCompanySize.FIFTY99EMPLOYEES:
            return fifty99employees()
        if self is PatchUsersRequestCompanySize.ONE_HUNDRED499EMPLOYEES:
            return one_hundred499employees()
        if self is PatchUsersRequestCompanySize.FIVE_HUNDRED999EMPLOYEES:
            return five_hundred999employees()
        if self is PatchUsersRequestCompanySize.ONE_THOUSAND4999EMPLOYEES:
            return one_thousand4999employees()
        if self is PatchUsersRequestCompanySize.FIVE_THOUSAND_EMPLOYEES:
            return five_thousand_employees()
        if self is PatchUsersRequestCompanySize.DONT_KNOW:
            return dont_know()
