# OpenAPI surface coverage — Schemas

The `schemas` region of
[`../openapi-surface-coverage.md`](../openapi-surface-coverage.md). Its boundary
is `## Scope` below; the selector grammar, the category rules, the settlement
classes and the ranking rubric are the index's, and this file carries only this
region's entries.

## Scope

Schema, Discriminator and XML objects, and every JSON Schema keyword wherever it
appears, including the ones 3.1 added.

## Entries

| key | oas | spec location | category | evidence | crozier sites | why bytes could move | settlement |
|---|---|---|---|---|---|---|---|

## Method notes

_Not yet classified._ The region pass records here what it measured, which `just
surface-census` invocations it ran, and anything it could not settle.
