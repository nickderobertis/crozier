...
| `link-description` | discards | 2 | link probe **re-run**: both descriptions emit 0 bytes Contract A measurement: **Committed Fern measurement:** [`link-description`](openapi-surface/probe-expected/link-description/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `link-requestBody` | discards + supply | 2 | link probe **re-run**: `get_thing` takes only `thing_id` Contract A measurement: **Committed Fern measurement:** [`link-requestBody`](openapi-surface/probe-expected/link-requestBody/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `link-server` | discards + supply | 2 | link probe **re-run**: neither host reaches `environment.py` Contract A measurement: **Committed Fern measurement:** [`link-server`](openapi-surface/probe-expected/link-server/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
...
| `xml-request` | discards | 2 | probe quoted in full, both exits 0; corpus declares `application/xml` 0 times Contract A measurement: **Committed Fern measurement:** [`xml-request`](openapi-surface/probe-expected/xml-request/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `xml-response` | discards | 2 | as `xml-request`; golden corroboration is `text/xml`, and is named as corroboration Contract A measurement: **Committed Fern measurement:** [`xml-response`](openapi-surface/probe-expected/xml-response/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
...
## Round 5 — the seven witness-supply probes

Seven rows of
[`openapi-surface-coverage.md`](openapi-surface-coverage.md#witness-supply-probes)'s
probe backlog were shapes whose *world-wide* witness search returned
`none-found`: `$dynamicAnchor`, `$dynamicRef`, `$vocabulary`, and the four IANA
HTTP authentication schemes `concealed`, `gnap`, `privatetoken` and `vapid`. No
document anywhere declares one, so no corpus row can ever pin them and nothing
...
[`openapi-surface/probes/`](openapi-surface/probes/), one file per measurement,
so every run below can be repeated rather than taken on trust. None is a corpus
fixture, none is proposed as one, and none sits under `tests/fixtures/` — the
corpus takes real-world specifications only
([`../tests/fixtures/AGENTS.md`](../tests/fixtures/AGENTS.md)).

**Every one of the seven is `discards`.** Fern accepts each shape, emits nothing
derived from it, and — for the shape itself — reports nothing. Where a document
declaring one of the four schemes *alone* is refused, the refusal is downstream
of the discard rather than a verdict on the scheme, and the paired document is
what says so; that is the same pair of measurements
[the four dropped schemes](#the-importer-drops-four-security-schemes-outright)
already records for `apiKey-query`, `apiKey-cookie`, `mutualTLS` and
`http-digest`, and these four join them.

**Each row stays convertible.** A `discards` verdict measured on a probe settles
what Fern does; it does not close the row against a real-world witness. Each
subsection below names what a registrable witness would have to be, and the day
one is found the row leaves `limitations` for `golden` under
[the classification precedence](openapi-surface-coverage.md#the-category-rules) —
the probe is the settlement available while the world supplies nothing, not a
replacement for a golden.

**Round 5 also ran crozier.** Every probe Fern generated cleanly was then
...
verdict, never `golden` — and where it diverged the repair went into `src/`,
never into this file. One divergence was found and repaired; it is recorded under
[The one crozier divergence Round 5 found](#the-one-crozier-divergence-round-5-found).

### Round 5 — `$dynamicAnchor` and `$dynamicRef`

**One probe, because the pair is only exercisable together.** `$dynamicRef` is a
reference and `$dynamicAnchor` is what it resolves to; a document declaring
either alone declares a dangling half. So
[`dollar-dynamic-recursion.yml`](openapi-surface/probes/dollar-dynamic-recursion.yml)
declares both, in the 2020-12 extensible-recursion idiom the keywords exist for:
    ...
    Tree:
      ...
      properties:
        ...
        children:
          type: array
          items:
            $dynamicRef: "#node"          # the dynamic reference
      required: [value]
      $defs:
        node:
          $dynamicAnchor: node            # the anchor it resolves to
          $ref: "#/components/schemas/Tree"
    StrictTree:
      title: StrictTree
      $dynamicAnchor: node                # redefined, so `#node` means StrictTree here
      $ref: "#/components/schemas/Tree"
      unevaluatedProperties: false
```

`fern check` **exit 0**, `All checks passed`; `fern generate` **exit 0**. Fern
emits a `Tree` model, and the dynamic reference reaches none of it:
...
`typing.Any` is the no-schema default, not a rendering of `#node`: the control
document
[`dollar-dynamic-recursion-control.yml`](openapi-surface/probes/dollar-dynamic-recursion-control.yml)
is the same document with the dynamic pair replaced by the static
`$ref: "#/components/schemas/Tree"` and the `$defs` dropped, and on it Fern emits

```python
from __future__ import annotations
class Tree(UniversalBaseModel):
    value: str
    children: typing.Optional[typing.List["Tree"]] = None
update_forward_refs(Tree)
```

— three artifacts the dynamic spelling does not produce. So the reference is not
resolved and its target is not modelled: `$defs.node` reaches no module, and
`$dynamicAnchor` reaches nothing either, `StrictTree` collapsing to the alias
`StrictTree = Tree` in both documents alike, with or without the anchor
redefinition that is supposed to change what `#node` means inside it. Verdict for
both rows: **discards**.

*Beside crozier:* crozier generated the probe and the control and **byte-matched
Fern on all 40 files of each** under the gate's normalization, after the repair
recorded below. `src/openapi.rs`'s `Schema` declares no `$dynamicAnchor`,
`$dynamicRef` or `$defs` field, so crozier drops all three the way Fern does.
...
[`dollar-vocabulary.yml`](openapi-surface/probes/dollar-vocabulary.yml) declares a
**custom schema dialect and the vocabulary map under it**, so the measurement is
of that feature rather than of a document that merely happens to be unusual: the
document-level `jsonSchemaDialect` names the dialect, a component schema declares
it with `$vocabulary`, and a second component schema is written against it.

```yaml
jsonSchemaDialect: https://probe.invalid/dialect/probe
...
    ProbeDialect:
      $id: https://probe.invalid/dialect/probe
      $schema: https://json-schema.org/draft/2020-12/schema
      $vocabulary:
        https://json-schema.org/draft/2020-12/vocab/core: true
        https://json-schema.org/draft/2020-12/vocab/applicator: true
        https://json-schema.org/draft/2020-12/vocab/validation: false
        https://probe.invalid/vocab/probe: true
      type: object
    ProbeResult:
      $schema: https://probe.invalid/dialect/probe
      type: object
      properties:
        ok: { type: boolean }
...
`fern check` **exit 0**, `All checks passed`; `fern generate` **exit 0**, no
diagnostic. Nothing emitted derives from the vocabulary map. The schema that
carries it becomes the bare-object default, `ProbeDialect = typing.Dict[str,
typing.Any]` — what a `type: object` with no properties emits with no
`$vocabulary` present at all — and the schema written *against* the dialect is
generated as if the dialect said nothing: the map declares the 2020-12 validation
vocabulary `false`, and `ProbeResult.count` is still `typing.Optional[int]`, typed
off `type: integer` exactly as it would be under the default dialect. The strings
`vocabulary` and `dialect` appear nowhere in the emitted SDK except in the module
and class names Fern derives from the schema *key* `ProbeDialect`. The
document-level `jsonSchemaDialect` reaches zero bytes too. Verdict: **discards**.

*Beside crozier:* crozier **byte-matched Fern on all 40 files** of this probe
under the gate's normalization. `src/openapi.rs`'s `Schema` declares no
`$vocabulary` field and the document model no `jsonSchemaDialect`, so crozier
ignores both the way Fern does.

*What a registrable witness would be:* an OpenAPI 3.1 document a real API
publishes as its own description, declaring `$vocabulary` on a Schema Object, at
a pinned credential-free direct spec URL under a redistribution-compatible
licence and accepted by `fern check`. The search found none — `$vocabulary` is a
*meta-schema* keyword, so an API description has no occasion to carry one — and
one turning up promotes this row to `golden`.

### Round 5 — `concealed`, `gnap`, `privatetoken` and `vapid`

Four rows, four IANA HTTP authentication scheme values Fern's importer does not
know: `concealed` (RFC 9729), `gnap` (RFC 9635), `privatetoken` (RFC 9577) and
`vapid` (RFC 8292). **Each is measured on two documents**, because the pair is
what distinguishes a scheme Fern *refuses* from one it silently drops: a document
declaring the scheme alone with every operation secured, and the same scheme
declared beside HTTP `bearer`, which Fern's importer supports. Both are committed
per scheme — `http-<scheme>-alone.yml` and `http-<scheme>-beside-bearer.yml`
under [`openapi-surface/probes/`](openapi-surface/probes/) — and each declares its
scheme in the one place the feature lives:

```yaml
  securitySchemes:
    ProbeScheme:
      type: http
      scheme: concealed        # gnap / privatetoken / vapid in the other three
```

All four schemes behave identically, and identically to the four
[already-measured dropped schemes](#the-importer-drops-four-security-schemes-outright):

| probe | `fern check` | `fern generate` | what Fern did |
|---|:--:|:--:|---|
| [`http-concealed-alone`](openapi-surface/probes/http-concealed-alone.yml) | **1** | **1** | `Service requires auth, but no auth is defined.` |
| [`http-gnap-alone`](openapi-surface/probes/http-gnap-alone.yml) | **1** | **1** | the same diagnostic |
| [`http-privatetoken-alone`](openapi-surface/probes/http-privatetoken-alone.yml) | **1** | **1** | the same diagnostic |
| [`http-vapid-alone`](openapi-surface/probes/http-vapid-alone.yml) | **1** | **1** | the same diagnostic |
| [`http-concealed-beside-bearer`](openapi-surface/probes/http-concealed-beside-bearer.yml) | 0 | 0 | a 39-file SDK generated from the **bearer** scheme — required `token: typing.Union[str, typing.Callable[[], str]]` → `Authorization` — with **zero occurrences of the string `concealed`** anywhere in the tree |
| [`http-gnap-beside-bearer`](openapi-surface/probes/http-gnap-beside-bearer.yml) | 0 | 0 | the same, zero occurrences of `gnap` |
| [`http-privatetoken-beside-bearer`](openapi-surface/probes/http-privatetoken-beside-bearer.yml) | 0 | 0 | the same, zero occurrences of `privatetoken` |
| [`http-vapid-beside-bearer`](openapi-surface/probes/http-vapid-beside-bearer.yml) | 0 | 0 | the same, zero occurrences of `vapid` |

The two answers together say what happened. The sole-scheme refusal is not a
verdict on the scheme: Fern's diagnostic names the *service*, not the scheme, and
it is what the importer having already dropped the scheme leaves behind — a
secured service with an empty scheme map. The paired document confirms it
directly: the scheme is accepted, generation succeeds, and the value leaves no
trace at all. Verdict for all four: **discards**.

*Beside crozier:* crozier generated each of the four paired documents and
**byte-matched Fern on all 39 files** of each under the gate's normalization.
`src/openapi.rs`'s `HttpAuthScheme` collapses every scheme that is not `bearer`
or `basic` into `Other` through its `#[serde(other)]` fallback, and `auth_model`
in `src/ir.rs` selects only `Bearer`/`Basic`, so crozier selects the bearer scheme
from the paired document exactly as Fern does. The four sole-scheme documents are
not compared: Fern emitted no output to compare against.

*What a registrable witness would be, for each of the four:* a document a real
API publishes as its own description, declaring `type: http` with that `scheme`
value **beside** a scheme Fern's importer supports — the sole-scheme form cannot
be registered, because Fern refuses it and a corpus row needs a golden — at a
pinned credential-free direct spec URL under a redistribution-compatible licence.
Its golden would pin precisely where the unsupported scheme is dropped, which is
what `http-dpop` and `http-mutual` are `golden` on today, and one turning up
promotes the row to `golden`.

### The one crozier divergence Round 5 found

The dynamic-recursion probe and its control both diverged from Fern in three
files — `reference.md`, `src/fern/client.py` and `src/fern/raw_client.py` — and
in one way: Fern typed `get_strict_tree` as returning `StrictTree`, and crozier
returned `Tree`, the schema the alias points at.

The cause was a load-time normalization in `src/openapi.rs`,
...
| the alias component | Fern's `get_a` returns |
|---|---|
| `AliasOfThing: {$ref: Thing}` | `AliasOfThing` |
| `… , title: AliasOfThing` | `AliasOfThing` |
...
The seven keys this round adds to the ledger. `eligible` and `verified` are the
witness search's own counts, and they are `0` and `0` for every row by
construction: these are exactly the rows whose world-wide search returned
`none-found`, which is why each is settled by a probe rather than by a corpus row.

| gap | eligible | verified | verdict | what the probe measured |
|---|---:|---:|---|---|
| `dollar-dynamic-anchor` | 0 | 0 | discards | `$dynamicAnchor` reaches no byte: `$defs.node` is emitted as no module, and `StrictTree` collapses to `StrictTree = Tree` with or without the anchor redefinition. Measured with `$dynamicRef` on one probe, because the pair is only exercisable together. Probe [`dollar-dynamic-recursion.yml`](openapi-surface/probes/dollar-dynamic-recursion.yml), `fern check` 0, `fern generate` 0; crozier byte-matches on all 40 files. Measured in [Round 5](#round-5--dynamicanchor-and-dynamicref) Committed Fern measurement: [`probe-expected/dollar-dynamic-anchor/`](openapi-surface/probe-expected/dollar-dynamic-anchor/). |
| `dollar-dynamic-ref` | 0 | 0 | discards | `$dynamicRef: "#node"` is not resolved: the array it types emits `typing.Optional[typing.List[typing.Any]]`, where the static control emits `typing.Optional[typing.List["Tree"]]` with `from __future__ import annotations` and `update_forward_refs(Tree)`. Same probe and control as `dollar-dynamic-anchor`. Measured in [Round 5](#round-5--dynamicanchor-and-dynamicref) Committed Fern measurement: [`probe-expected/dollar-dynamic-ref/`](openapi-surface/probe-expected/dollar-dynamic-ref/). |
| `dollar-vocabulary` | 0 | 0 | discards | a custom dialect declaring `$vocabulary` reaches no byte: the declaring schema emits the bare-object default `typing.Dict[str, typing.Any]`, and a schema written against the dialect keeps `count: typing.Optional[int]` though the map declares the 2020-12 validation vocabulary `false`. Probe [`dollar-vocabulary.yml`](openapi-surface/probes/dollar-vocabulary.yml), `fern check` 0 `All checks passed`, `fern generate` 0; crozier byte-matches on all 40 files. Measured in [Round 5](#round-5--vocabulary) Committed Fern measurement: [`probe-expected/dollar-vocabulary/`](openapi-surface/probe-expected/dollar-vocabulary/). |
| `http-concealed` | 0 | 0 | discards | the importer drops the scheme outright. Declared alone with every operation secured, `fern check` and `fern generate` both exit **1** with `Service requires auth, but no auth is defined.`; declared beside HTTP `bearer`, both exit 0 and the 39-file SDK is generated from the bearer scheme with zero occurrences of `concealed`. Probes [`http-concealed-alone.yml`](openapi-surface/probes/http-concealed-alone.yml) and [`http-concealed-beside-bearer.yml`](openapi-surface/probes/http-concealed-beside-bearer.yml); crozier byte-matches the paired document on all 39 files. Measured in [Round 5](#round-5--concealed-gnap-privatetoken-and-vapid) Contract A measurement: **Committed Fern measurement:** [`http-concealed`](openapi-surface/probe-expected/http-concealed/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `http-gnap` | 0 | 0 | discards | as `http-concealed`, on [`http-gnap-alone.yml`](openapi-surface/probes/http-gnap-alone.yml) and [`http-gnap-beside-bearer.yml`](openapi-surface/probes/http-gnap-beside-bearer.yml): **1**/**1** with the same diagnostic alone, 0/0 beside `bearer` with zero occurrences of `gnap`; crozier byte-matches the paired document on all 39 files. Measured in [Round 5](#round-5--concealed-gnap-privatetoken-and-vapid) Contract A measurement: **Committed Fern measurement:** [`http-gnap`](openapi-surface/probe-expected/http-gnap/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `http-privatetoken` | 0 | 0 | discards | as `http-concealed`, on [`http-privatetoken-alone.yml`](openapi-surface/probes/http-privatetoken-alone.yml) and [`http-privatetoken-beside-bearer.yml`](openapi-surface/probes/http-privatetoken-beside-bearer.yml): **1**/**1** with the same diagnostic alone, 0/0 beside `bearer` with zero occurrences of `privatetoken`; crozier byte-matches the paired document on all 39 files. Measured in [Round 5](#round-5--concealed-gnap-privatetoken-and-vapid) Contract A measurement: **Committed Fern measurement:** [`http-privatetoken`](openapi-surface/probe-expected/http-privatetoken/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `http-vapid` | 0 | 0 | discards | as `http-concealed`, on [`http-vapid-alone.yml`](openapi-surface/probes/http-vapid-alone.yml) and [`http-vapid-beside-bearer.yml`](openapi-surface/probes/http-vapid-beside-bearer.yml): **1**/**1** with the same diagnostic alone, 0/0 beside `bearer` with zero occurrences of `vapid`; crozier byte-matches the paired document on all 39 files. Measured in [Round 5](#round-5--concealed-gnap-privatetoken-and-vapid) Contract A measurement: **Committed Fern measurement:** [`http-vapid`](openapi-surface/probe-expected/http-vapid/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
...
**"Byte-identical" below is the pair compared as this repository compares a
golden**: every `.py` on both sides through `crozier internal-strip`, every other
file — `README.md`, `reference.md`, `pyproject.toml`, `.fern/metadata.json` — as
Fern wrote it. Fern's own `.py` comment is the constant *"This file was
auto-generated by Fern from our API Definition."*, so what stripping removes is
that line and nothing a document could vary.
...
Every row here is `eligible` **0**, which is why each is settled by a probe rather
than by a corpus row, and `verified` is what separates the routes they took: a row
above zero has a real witness this corpus cannot use and is on
[route 2](openapi-surface-coverage.md#the-settlement-rule-as-amended), while a row
at **0** whose search left a required source unanswered is on route 3, which
claims nothing about the world at all. Round 5's rows took neither route: their
searches returned `none-found`.

| gap | eligible | verified | verdict | what the probe measured |
|---|---:|---:|---|---|
| `http-hoba` | 0 | 1 | discards | **Committed Fern measurement:** [`http-hoba`](openapi-surface/probe-expected/http-hoba/). The importer drops the scheme outright. Declared alone with every operation secured, `fern check` and `fern generate` both exit **1** with `Service requires auth, but no auth is defined.`; declared beside HTTP `bearer`, both exit 0 with `All checks passed` and the 39-file SDK is generated from the bearer scheme with zero occurrences of `hoba`. Probes [`http-hoba-alone.yml`](openapi-surface/probes/http-hoba-alone.yml) and [`http-hoba-beside-bearer.yml`](openapi-surface/probes/http-hoba-beside-bearer.yml); crozier byte-matches the paired document on all 39 files. Measured in [Round 6](#round-6--security) |
| `http-oauth` | 0 | 3 | discards | as `http-hoba`, on [`http-oauth-alone.yml`](openapi-surface/probes/http-oauth-alone.yml) and [`http-oauth-beside-bearer.yml`](openapi-surface/probes/http-oauth-beside-bearer.yml): **1**/**1** with the same diagnostic alone — the refusal the Noun Project bundle already met, reproduced on a document that declares nothing else — 0/0 beside `bearer` with zero occurrences of `oauth`; crozier byte-matches the paired document on all 39 files. Measured in [Round 6](#round-6--security) Contract A measurement: **Committed Fern measurement:** [`http-oauth`](openapi-surface/probe-expected/http-oauth/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `http-scram-sha-1` | 0 | 1 | discards | as `http-hoba`, on [`http-scram-sha-1-alone.yml`](openapi-surface/probes/http-scram-sha-1-alone.yml) and [`http-scram-sha-1-beside-bearer.yml`](openapi-surface/probes/http-scram-sha-1-beside-bearer.yml): **1**/**1** with the same diagnostic alone, 0/0 beside `bearer` with zero occurrences of `scram-sha-1`; crozier byte-matches the paired document on all 39 files. Measured in [Round 6](#round-6--security) Contract A measurement: **Committed Fern measurement:** [`http-scram-sha-1`](openapi-surface/probe-expected/http-scram-sha-1/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `http-scram-sha-256` | 0 | 1 | discards | as `http-hoba`, on [`http-scram-sha-256-alone.yml`](openapi-surface/probes/http-scram-sha-256-alone.yml) and [`http-scram-sha-256-beside-bearer.yml`](openapi-surface/probes/http-scram-sha-256-beside-bearer.yml): **1**/**1** with the same diagnostic alone, 0/0 beside `bearer` with zero occurrences of `scram-sha-256`; crozier byte-matches the paired document on all 39 files. Measured in [Round 6](#round-6--security) Contract A measurement: **Committed Fern measurement:** [`http-scram-sha-256`](openapi-surface/probe-expected/http-scram-sha-256/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
...
| `components-pathItems` | 6 | 7 | discards | `components.pathItems` reaches the SDK only through a Path Item `$ref`, which Fern discards; the one candidate that loses nothing witnesses the *declaration* alone Contract A measurement: **Committed Fern measurement:** [`components-pathItems`](openapi-surface/probe-expected/components-pathItems/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `const-boolean` | 13 | 13 | discards | measured on two of its own candidate specifications, both generating at exit 0: `omersiar/ript` emits `has_more: bool` for `{type: boolean, const: false}`, and `MarkiNote` emits `success: typing.Optional[bool] = None` for `{const: true, default: true, type: boolean}` — the `default` is dropped with the `const`, so the client can construct the flag `False`. A probe supplies the control: a `type: string` `const` in the same document becomes the single-member `StrEnum` `tamoss` (row 69) pins, while the boolean one becomes a bare `bool`. No corpus document declares a boolean `const` at all. Measured in [Round 4](#round-4--schema-shapes) Committed Fern measurement: [`probe-expected/const-boolean/`](openapi-surface/probe-expected/const-boolean/). |
| `const-integer` | 7 | 8 | discards | as `const-boolean`, on three candidate specifications that all generate at exit 0: `omersiar/ript` emits `count: int`, `total: int` and `page: int` for `const` 0, 0 and 1 — indistinguishable from the `limit: int` beside them, which declares `minimum: 1` instead — and `MarkiNote` and `tltv-org/protocol` each emit a bare `int` for their own integer `const`. No corpus document declares an integer `const` at all. Measured in [Round 4](#round-4--schema-shapes) Committed Fern measurement: [`probe-expected/const-integer/`](openapi-surface/probe-expected/const-integer/). |
| `cookie-array` | 0 | 3 | discards + supply | cookie parameters are dropped from the client entirely; and 0 eligible of 3 verified — 3× licence untiered; committed Fern measurement: [`cookie-array`](openapi-surface/probe-expected/cookie-array/) |
| `cookie-object` | 1 | 6 | discards + supply | as `cookie-array`; 1 eligible of 6 verified — 5× licence untiered; committed Fern measurement: [`cookie-object`](openapi-surface/probe-expected/cookie-object/) |
...
| `enum-member-float` | 2 | 2 | discards + supply | Fern accepts the schema and silently strips the enum, emitting `Kind = float` with no members; and only 2 eligible, pool exhausted Committed Fern measurement: [`probe-expected/enum-member-float/`](openapi-surface/probe-expected/enum-member-float/). |
| `enum-member-object` | 1 | 1 | discards + supply | as above, emitting `Kind = typing.Dict[str, typing.Any]`; only 1 eligible, pool exhausted Committed Fern measurement: [`probe-expected/enum-member-object/`](openapi-surface/probe-expected/enum-member-object/). |
| `explode-true-simple-header` | 0 | 0 | ignores | `fern check` and `fern generate` both exit 0; the committed control proves `explode` reaches no generated byte. This pinned measurement supersedes the earlier prose refusal verdict; no real-world candidate was carried end to end. Committed Fern measurement: [`explode-true-simple-header`](openapi-surface/probe-expected/explode-true-simple-header/) |
| `explode-true-simple-path` | 4 | 4 | ignores | the parameter is emitted but `explode` is ignored entirely on a simple-style path parameter; committed Fern measurement: [`explode-true-simple-path`](openapi-surface/probe-expected/explode-true-simple-path/) |
| `extension-callback` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-callback`](openapi-surface/probe-expected/extension-callback/) and [`extension-callback-control`](openapi-surface/probe-expected/extension-callback-control/); the two complete SDK trees agree byte for byte. |
| `extension-components` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-components`](openapi-surface/probe-expected/extension-components/) and [`extension-components-control`](openapi-surface/probe-expected/extension-components-control/); the two complete SDK trees agree byte for byte. |
| `extension-discriminator` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-discriminator`](openapi-surface/probe-expected/extension-discriminator/) and [`extension-discriminator-control`](openapi-surface/probe-expected/extension-discriminator-control/); the two complete SDK trees agree byte for byte. |
| `extension-encoding` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-encoding`](openapi-surface/probe-expected/extension-encoding/) and [`extension-encoding-control`](openapi-surface/probe-expected/extension-encoding-control/); the two complete SDK trees agree byte for byte. |
| `extension-example` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-example`](openapi-surface/probe-expected/extension-example/) and [`extension-example-control`](openapi-surface/probe-expected/extension-example-control/); the two complete SDK trees agree byte for byte. |
| `extension-external-docs` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-external-docs`](openapi-surface/probe-expected/extension-external-docs/) and [`extension-external-docs-control`](openapi-surface/probe-expected/extension-external-docs-control/); the two complete SDK trees agree byte for byte. |
| `extension-header` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-header`](openapi-surface/probe-expected/extension-header/) and [`extension-header-control`](openapi-surface/probe-expected/extension-header-control/); the two complete SDK trees agree byte for byte. |
| `extension-license` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-license`](openapi-surface/probe-expected/extension-license/) and [`extension-license-control`](openapi-surface/probe-expected/extension-license-control/); the two complete SDK trees agree byte for byte. |
| `extension-link` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-link`](openapi-surface/probe-expected/extension-link/) and [`extension-link-control`](openapi-surface/probe-expected/extension-link-control/); the two complete SDK trees agree byte for byte. |
| `extension-media-type` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-media-type`](openapi-surface/probe-expected/extension-media-type/) and [`extension-media-type-control`](openapi-surface/probe-expected/extension-media-type-control/); the two complete SDK trees agree byte for byte. |
| `extension-oauth-flow` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-oauth-flow`](openapi-surface/probe-expected/extension-oauth-flow/) and [`extension-oauth-flow-control`](openapi-surface/probe-expected/extension-oauth-flow-control/); the two complete SDK trees agree byte for byte. |
| `extension-oauth-flows` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-oauth-flows`](openapi-surface/probe-expected/extension-oauth-flows/) and [`extension-oauth-flows-control`](openapi-surface/probe-expected/extension-oauth-flows-control/); the two complete SDK trees agree byte for byte. |
| `extension-request-body` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-request-body`](openapi-surface/probe-expected/extension-request-body/) and [`extension-request-body-control`](openapi-surface/probe-expected/extension-request-body-control/); the two complete SDK trees agree byte for byte. |
| `extension-responses` | 0 | 0 | ignores | Contract A differential measurement at Fern CLI 5.67.1 / Python SDK 5.20.0: **Committed Fern measurement:** [`extension-responses`](openapi-surface/probe-expected/extension-responses/) and [`extension-responses-control`](openapi-surface/probe-expected/extension-responses-control/); the two complete SDK trees agree byte for byte. |
...
| `link-requestBody` | 2 | 3 | discards + supply | probed directly: a Link `requestBody` expression emits nothing — the target operation's generated method takes only its path parameter. No corpus document declares the field. 2 eligible of 3 verified, short of a primary plus two backups. Measured in [Round 4](#round-4--links-and-encoding) Contract A measurement: **Committed Fern measurement:** [`link-requestBody`](openapi-surface/probe-expected/link-requestBody/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `link-server` | 1 | 3 | discards + supply | probed directly: a Link `server` reaches neither the environment enum — which keeps the root URL alone — nor any call site. No corpus document declares the field. 1 eligible of 3 verified, so the pool holds one real-world witness where three are needed. Measured in [Round 4](#round-4--links-and-encoding) Contract A measurement: **Committed Fern measurement:** [`link-server`](openapi-surface/probe-expected/link-server/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `matrix-array` | 0 | 4 | discards + licence | Fern discards the `matrix` style and renders `str(list)` into the path segment; 0 eligible of 4 verified — 4× licence untiered; committed Fern measurement: [`matrix-array`](openapi-surface/probe-expected/matrix-array/) |
| `matrix-object` | 0 | 0 | discards | Fern discards the `matrix` style; no candidate carried end to end at all; committed Fern measurement: [`matrix-object`](openapi-surface/probe-expected/matrix-object/) |
...
| `nonascii-info-title` | 5 | 5 | ignores | probed directly: `info.title: 推奨データセット` is accepted and never reaches an identifier, so nothing emitted derives from it; committed Fern measurement: [`nonascii-info-title`](openapi-surface/probe-expected/nonascii-info-title/) |
| `nonascii-operationId` | 1 | 1 | crashes + supply | the generator emits invalid Python for a genuinely non-ASCII operationId; the one surviving candidate carries a Latin-1 accented one that folds, so it does not carry the shape; committed Fern measurement: [`nonascii-operationId`](openapi-surface/probe-expected/nonascii-operationId.fern-refusal.txt) |
...
| `param-content-cookie` | 0 | 0 | discards | cookie parameters are dropped from the client entirely; no candidate carried end to end at all; committed Fern measurement: [`param-content-cookie`](openapi-surface/probe-expected/param-content-cookie/) |
| `param-content-path` | 0 | 0 | discards | Fern collapses a `content:`-typed parameter to a bare `str`, losing the declared object; no candidate carried end to end at all; committed Fern measurement: [`param-content-path`](openapi-surface/probe-expected/param-content-path/) |
| `pathitem-ref` | 7 | 7 | implements | The earlier single-file screens lost relative references because their targets were absent. Fern 5.20.0 accepts Raybot's complete pinned tree and emits clients from its 23 sibling Path Items: `/version` becomes `src/fern/version/client.py` and `/health` becomes `src/fern/health/client.py`. Corpus row 140 byte-matches. The old remote-URL observation remains a separate resolving form. |
| `pipeDelimited-array` | 7 | 7 | discards | Fern discards the declared style: the array emits as an httpx repeated key, `"probeParam": probe_param`, not pipe-joined; committed Fern measurement: [`pipeDelimited-array`](openapi-surface/probe-expected/pipeDelimited-array/) |
| `pipeDelimited-object` | 1 | 7 | discards + supply | Fern discards the declared style; object query parameters are flattened regardless of it; 1 eligible of 7 verified — 6× licence untiered; committed Fern measurement: [`pipeDelimited-object`](openapi-surface/probe-expected/pipeDelimited-object/) |
| `range-1XX` | 1 | 6 | discards + supply | a ranged response yields no error class at all; the operation falls through to `ApiError`; 1 eligible of 6 verified — 5× licence untiered Contract A measurement: **Committed Fern measurement:** [`range-1XX`](openapi-surface/probe-expected/range-1XX/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `range-3XX` | 3 | 5 | discards | a ranged response yields no error class at all Contract A measurement: **Committed Fern measurement:** [`range-3XX`](openapi-surface/probe-expected/range-3XX/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `range-4XX` | 5 | 6 | discards | a ranged response yields no error class at all Contract A measurement: **Committed Fern measurement:** [`range-4XX`](openapi-surface/probe-expected/range-4XX/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `range-5XX` | 4 | 4 | discards | a ranged response yields no error class at all Contract A measurement: **Committed Fern measurement:** [`range-5XX`](openapi-surface/probe-expected/range-5XX/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `relative-file-ref` | 2 | 2 | implements | Fern 5.20.0 generates from a relative-file Path Item when its target is present: Raybot corpus row 140 pins `openapi.yml` and all 35 sibling documents at one revision; its `/version` → `paths/version.yml` reference yields `src/fern/version/client.py` and `/health` → `paths/health.yml` yields `src/fern/health/client.py`. Both byte-match crozier in the committed golden. The earlier two eligible documents were fetched alone, so their missing targets explain their endpoint-free output rather than a discard of a resolvable reference. |
| `server-description-multiword` | 15 | 16 | discards | a multi-word server description reaches no identifier: all 18 goldens whose first root server carries one emit `DEFAULT`, while the four whose description is the single word `Production` emit `PRODUCTION`. Measured in [Round 4](#round-4--servers-and-xml); committed Fern measurement: [`server-description-multiword`](openapi-surface/probe-expected/server-description-multiword/) |
...
| `servers-three-levels` | 1 | 5 | discards | probed directly: with a root, a Path Item and an Operation server in one document, `fern check` and `fern generate` both exit 0 and only the root URL is emitted. Measured in [Round 4](#round-4--servers-and-xml); committed Fern measurement: [`servers-three-levels`](openapi-surface/probe-expected/servers-three-levels/) |
| `spaceDelimited-object` | 0 | 6 | discards + licence | Fern discards the declared style; object query parameters are flattened regardless of it; 0 eligible of 6 verified — 6× licence untiered; committed Fern measurement: [`spaceDelimited-object`](openapi-surface/probe-expected/spaceDelimited-object/) |
| `trace-operation` | 2 | 5 | discards + supply | Fern emits no client method for `trace`; 2 eligible of 5 verified — 3× licence tier Q; committed Fern measurement: [`trace-operation`](openapi-surface/probe-expected/trace-operation/) |
...
| `xml-request` | 18 | 20 | discards | probed directly: a **required** `application/xml` request body is dropped whole — the method takes no body argument and the call site sends none. No committed golden declares `application/xml`, so this rests on the probe. Measured in [Round 4](#round-4--servers-and-xml) Contract A measurement: **Committed Fern measurement:** [`xml-request`](openapi-surface/probe-expected/xml-request/), Fern CLI 5.67.1 and Python SDK 5.20.0. |
| `xml-response` | 20 | 24 | discards | probed directly: an `application/xml` response schema is dropped whole — the method returns `HttpResponse[None]` and never parses the body. No committed golden declares `application/xml`, so this rests on the probe. Measured in [Round 4](#round-4--servers-and-xml) Contract A measurement: **Committed Fern measurement:** [`xml-response`](openapi-surface/probe-expected/xml-response/), Fern CLI 5.67.1 and Python SDK 5.20.0. |

### Historical APIs.guru probe corroboration

The Round 7 probes and generated expectations remain as non-settling corroboration
of Fern behavior. Their input was only the historical one-source
[`apis-guru-gap-witnesses.tsv`](openapi-surface/apis-guru-gap-witnesses.tsv): it
may establish an APIs.guru answer or candidate, but its `none-found` cells are not
the outcome of the required seven-source search and settle no row.
...
### Unreachable schema positions

These three schema positions have no generated Python SDK representation. Each
probe and its control differ only in the named feature; Fern CLI 5.67.1 and
`fernapi/fern-python-sdk` 5.20.0 accept and generate both, and the complete
comment-stripped trees are byte-identical. The manifest records and gates both
trees, with crozier compared against each.

| key | verdict | committed Fern measurement |
|---|---|---|
...
| `xml-wrapped` | ignores | [`xml-wrapped/`](openapi-surface/probe-expected/xml-wrapped/) and [`xml-wrapped-control/`](openapi-surface/probe-expected/xml-wrapped-control/) |
