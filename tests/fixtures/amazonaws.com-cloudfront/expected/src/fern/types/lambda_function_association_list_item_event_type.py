...
class LambdaFunctionAssociationListItemEventType(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is LambdaFunctionAssociationListItemEventType.VIEWER_REQUEST:
            return viewer_request()
        if self is LambdaFunctionAssociationListItemEventType.VIEWER_RESPONSE:
            return viewer_response()
        if self is LambdaFunctionAssociationListItemEventType.ORIGIN_REQUEST:
            return origin_request()
        if self is LambdaFunctionAssociationListItemEventType.ORIGIN_RESPONSE:
