...
| **total** | **402** | **272** | **42** | **88** | **38** | **30** | **20** |

The walk enumerated **402** features and landed each in exactly one category:
**272** `golden`, **42** `limitations`, **88** `gap`. The `gap` column splits by
settlement class into **38** `FIXTURE`, **30** `PROBE` and **20** `UNREACHABLE`.

**What the `gap` count means.** 88 is the number of OpenAPI shapes for which
...
nothing contradicts whatever crozier does. `just check` is green over all 88
...
The two backlogs below are the other 68.
...
**The median blind-spot count of this list is 0** — 26 of the 38 entries name no
...
| 4 | [`content-encoding`](openapi-surface/schemas.md) | `schemas` | **0** (none) | **0** (no `src/` file) | **2** (types/, raw_client.py) | **0** |
...
| 13 | [`format-duration`](openapi-surface/schemas.md) | `schemas` | **0** (none) | **0** (no `src/` file) | **1** (types/) | **0** |
...
own measurement; it refines it, because 26 of the 38 ranked entries reach no
