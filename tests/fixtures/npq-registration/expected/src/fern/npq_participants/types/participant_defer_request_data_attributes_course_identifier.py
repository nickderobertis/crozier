...
class ParticipantDeferRequestDataAttributesCourseIdentifier(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_SENIOR_LEADERSHIP:
            return npq_senior_leadership()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_HEADSHIP:
            return npq_headship()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_EXECUTIVE_LEADERSHIP:
            return npq_executive_leadership()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_EARLY_YEARS_LEADERSHIP:
            return npq_early_years_leadership()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_LEADING_TEACHING:
            return npq_leading_teaching()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_LEADING_BEHAVIOUR_CULTURE:
            return npq_leading_behaviour_culture()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_LEADING_TEACHING_DEVELOPMENT:
            return npq_leading_teaching_development()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_LEADING_LITERACY:
            return npq_leading_literacy()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_LEADING_PRIMARY_MATHEMATICS:
            return npq_leading_primary_mathematics()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_ADDITIONAL_SUPPORT_OFFER:
            return npq_additional_support_offer()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_EARLY_HEADSHIP_COACHING_OFFER:
            return npq_early_headship_coaching_offer()
        if self is ParticipantDeferRequestDataAttributesCourseIdentifier.NPQ_SENCO:
            return npq_senco()
