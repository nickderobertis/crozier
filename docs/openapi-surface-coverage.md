...
`tests/fixtures/<name>/openapi.*` documents, and the 154 `link-ok` documents
...
- **`just surface-census`**, for the classifications and for criterion 4. The
  ...
  It reads **186** registered sources, of which
  **169** carry a committed golden. `document-paths`'s own evidence cells are
...
The witness searches' registrations add fourteen sources — corpus rows 144 to
157, `paloalto-cspm-alerts`, `paloalto-cspm-reports`,
...
`spendesk`, `billie` and `alma-france` (32 vendored plus 154 `link-ok`) — and promote seven `schemas` features to `golden`:
