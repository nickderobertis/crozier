...
class ListAgentsForBlockRequestIncludeItem(enum.StrEnum):
    ...
    def visit(
        ...
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_BLOCKS:
            return agent_blocks()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_IDENTITIES:
            return agent_identities()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_MANAGED_GROUP:
            return agent_managed_group()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_PENDING_APPROVAL:
            return agent_pending_approval()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_SECRETS:
            return agent_secrets()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_SOURCES:
            return agent_sources()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_TAGS:
            return agent_tags()
        if self is ListAgentsForBlockRequestIncludeItem.AGENT_TOOLS:
            return agent_tools()
