...
| media-type-range | both | Media Type Object content-map key | golden | Census object-model map-key walk for a `type/*` range other than `*/*`: **six** declarations, all in `torrentarr` (corpus row 127) and all `image/*` on a `200` response over `{type: string, format: binary}` — `…/artist/{artist_id}/thumbnail`, `…/movie/{id}/thumbnail` and `…/series/{id}/thumbnail`, each under both `/api` and `/web` — against zero in every other registered source (the only other key holding a `*` is `eozilla`'s malformed `/*`, classified below). Fern emits each as a streamed `typing.Iterator[bytes]` method under a `httpx_client.stream(...)` call and crozier reproduces the golden byte for byte. **The witness reaches the response side of the shape and not the request side, and this row is `golden` on what it reaches.** It exercises `src/ir.rs`'s `is_binary_response`, where an `image/*` key matches the `starts_with("image/")` test, and `response_schema`'s key-agnostic first-media fallback into the method's return type and its `reference.md` entry. It reaches none of the six request-side reads a range-keyed *body* would — the binary-body scan in `resolve_request_body`, `reference_body_example`, `selected_json_request_media`, `request_body_ignored` and `has_dispatchable_media`, and `src/emit.rs`'s bytes-body `content-type` header in `append_request_call_args` — because no registered source keys a request body on a range. | | | |
...
inconsistency. `media-type-range`'s registration has since landed, on the
`Feramance/Torrentarr` document this table records beside `gotson/komga`: it
declares the shape six times to komga's one and additionally carries
[`document-paths.md`](document-paths.md)'s `duplicate-normalized-paths`, so one
corpus row closed a row in each of two regions.
