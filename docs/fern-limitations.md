...
|---|---:|---:|---|---|
| `dollar-dynamic-anchor` | 0 | 0 | discards | `$dynamicAnchor` reaches no byte: `$defs.node` is emitted as no module, and `StrictTree` collapses to `StrictTree = Tree` with or without the anchor redefinition. Measured with `$dynamicRef` on one probe, because the pair is only exercisable together. Probe [`dollar-dynamic-recursion.yml`](openapi-surface/probes/dollar-dynamic-recursion.yml), `fern check` 0, `fern generate` 0; crozier byte-matches on all 40 files. Measured in [Round 5](#round-5--dynamicanchor-and-dynamicref) |
| `dollar-dynamic-ref` | 0 | 0 | discards | `$dynamicRef: "#node"` is not resolved: the array it types emits `typing.Optional[typing.List[typing.Any]]`, where the static control emits `typing.Optional[typing.List["Tree"]]` with `from __future__ import annotations` and `update_forward_refs(Tree)`. Same probe and control as `dollar-dynamic-anchor`. Measured in [Round 5](#round-5--dynamicanchor-and-dynamicref) |
| `dollar-vocabulary` | 0 | 0 | discards | a custom dialect declaring `$vocabulary` reaches no byte: the declaring schema emits the bare-object default `typing.Dict[str, typing.Any]`, and a schema written against the dialect keeps `count: typing.Optional[int]` though the map declares the 2020-12 validation vocabulary `false`. Probe [`dollar-vocabulary.yml`](openapi-surface/probes/dollar-vocabulary.yml), `fern check` 0 `All checks passed`, `fern generate` 0; crozier byte-matches on all 40 files. Measured in [Round 5](#round-5--vocabulary) |
