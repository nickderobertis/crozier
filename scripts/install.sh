...
#   CROZIER_RELEASE_BASE_URL=https://mirror.example/crozier sh install.sh
...
#
# Equivalent environment variables: CROZIER_VERSION, CROZIER_INSTALL_DIR,
# CROZIER_RELEASE_BASE_URL, CROZIER_CHECKSUM_BASE_URL.
...
# use `cargo install crozier --locked` or `pip install crozier`.
#
...
BIN="crozier"
# Overridden to `crozier.exe` on Windows targets by detect_target.
BIN_FILE="$BIN"

# Canonical release host. The archive may be fetched from a mirror (see
# CROZIER_RELEASE_BASE_URL), but checksums default to this host so the integrity
...
usage() {
    cat >&2 <<EOF
Install the prebuilt crozier binary.
...
Environment: CROZIER_VERSION, CROZIER_INSTALL_DIR, CROZIER_RELEASE_BASE_URL,
CROZIER_CHECKSUM_BASE_URL, GITHUB_TOKEN.
EOF
}
...
verify_archive() {
 ...
 'pip install sigstore'), or set CROZIER_CHECKSUM_BASE_URL to a root the mirror\
 does not control."
    fi
...
main() {
    version="${CROZIER_VERSION:-}"
    bindir="${CROZIER_INSTALL_DIR:-}"
    release_base="${CROZIER_RELEASE_BASE_URL:-}"
    checksum_base="${CROZIER_CHECKSUM_BASE_URL:-}"
    ...
    # extension with `.sha256` (not appending), e.g. crozier-v0.1.0-<t>.sha256.
    ...
    tmp="$(mktemp -d 2>/dev/null || mktemp -d -t crozier)" \
    ...
    say "run '${BIN} --version' to confirm the install."
}
