

import datetime as dt
import typing

import pydantic
import typing_extensions
from ..core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ..core.serialization import FieldMetadata
from .destiny_destiny_progression import DestinyDestinyProgression


class DestinyEntitiesVendorsDestinyVendorComponent(UniversalBaseModel):
    """
    This component contains essential/summary information about the vendor.
    """

    can_purchase: typing_extensions.Annotated[
        typing.Optional[bool],
        FieldMetadata(alias="canPurchase"),
        pydantic.Field(alias="canPurchase", description="If True, you can purchase from the Vendor."),
    ] = None
    """
    If True, you can purchase from the Vendor.
    """

    enabled: typing.Optional[bool] = pydantic.Field(default=None)
    """
    If True, the Vendor is currently accessible. 
    If False, they may not actually be visible in the world at the moment.
    """

    next_refresh_date: typing_extensions.Annotated[
        typing.Optional[dt.datetime],
        FieldMetadata(alias="nextRefreshDate"),
        pydantic.Field(
            alias="nextRefreshDate",
            description="The date when this vendor's inventory will next rotate/refresh.\r\nNote that this is distinct from the date ranges that the vendor is visible/available in-game: this field indicates the specific time when the vendor's available items refresh and rotate, regardless of whether the vendor is actually available at that time. Unfortunately, these two values may be (and are, for the case of important vendors like Xur) different.\r\nIssue https://github.com/Bungie-net/api/issues/353 is tracking a fix to start providing visibility date ranges where possible in addition to this refresh date, so that all important dates for vendors are available for use.",
        ),
    ] = None
    """
    The date when this vendor's inventory will next rotate/refresh.
    Note that this is distinct from the date ranges that the vendor is visible/available in-game: this field indicates the specific time when the vendor's available items refresh and rotate, regardless of whether the vendor is actually available at that time. Unfortunately, these two values may be (and are, for the case of important vendors like Xur) different.
    Issue https://github.com/Bungie-net/api/issues/353 is tracking a fix to start providing visibility date ranges where possible in addition to this refresh date, so that all important dates for vendors are available for use.
    """

    progression: typing.Optional[DestinyDestinyProgression] = pydantic.Field(default=None)
    """
    If the Vendor has a related Reputation, this is the Progression data that represents the character's Reputation level with this Vendor.
    """

    seasonal_rank: typing_extensions.Annotated[
        typing.Optional[int],
        FieldMetadata(alias="seasonalRank"),
        pydantic.Field(
            alias="seasonalRank",
            description="If this vendor has a seasonal rank, this will be the calculated value of that rank. How nice is that? I mean, that's pretty sweeet. It's a whole 32 bit integer.",
        ),
    ] = None
    """
    If this vendor has a seasonal rank, this will be the calculated value of that rank. How nice is that? I mean, that's pretty sweeet. It's a whole 32 bit integer.
    """

    vendor_hash: typing_extensions.Annotated[
        typing.Optional[int],
        FieldMetadata(alias="vendorHash"),
        pydantic.Field(
            alias="vendorHash",
            description="The unique identifier for the vendor. Use it to look up their DestinyVendorDefinition.",
        ),
    ] = None
    """
    The unique identifier for the vendor. Use it to look up their DestinyVendorDefinition.
    """

    vendor_location_index: typing_extensions.Annotated[
        typing.Optional[int],
        FieldMetadata(alias="vendorLocationIndex"),
        pydantic.Field(
            alias="vendorLocationIndex",
            description="An index into the vendor definition's \"locations\" property array, indicating which location they are at currently. If -1, then the vendor has no known location (and you may choose not to show them in your UI as a result. I mean, it's your bag honey)",
        ),
    ] = None
    """
    An index into the vendor definition's "locations" property array, indicating which location they are at currently. If -1, then the vendor has no known location (and you may choose not to show them in your UI as a result. I mean, it's your bag honey)
    """

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
