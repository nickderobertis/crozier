...
async def main() -> None:
    await client.events.watch_events()
...
client = FernApi(...)
response = client.events.with_raw_response.watch_events(...)
...
```python
client.events.watch_events(..., request_options={
    "max_retries": 1
})
```
