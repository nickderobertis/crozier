...
`tests/fixtures/<name>/openapi.*` documents, and the 105 `link-ok` documents
...
- **`just surface-census`**, for the classifications and for criterion 4. The
  ...
  rather than restated here. It reads **136** registered sources, of which
  **119** carry a committed golden.
...
reports zero across all 136 registered sources. So the inconsistency between the
