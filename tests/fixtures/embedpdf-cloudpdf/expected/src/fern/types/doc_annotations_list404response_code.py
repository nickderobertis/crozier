...
class DocAnnotationsList404ResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocAnnotationsList404ResponseCode.UNKNOWN:
            return unknown()
        if self is DocAnnotationsList404ResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocAnnotationsList404ResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocAnnotationsList404ResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocAnnotationsList404ResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocAnnotationsList404ResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocAnnotationsList404ResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocAnnotationsList404ResponseCode.ABORTED:
            return aborted()
        if self is DocAnnotationsList404ResponseCode.NETWORK:
            return network()
        if self is DocAnnotationsList404ResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocAnnotationsList404ResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocAnnotationsList404ResponseCode.NOT_FOUND:
            return not_found()
        if self is DocAnnotationsList404ResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocAnnotationsList404ResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocAnnotationsList404ResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocAnnotationsList404ResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocAnnotationsList404ResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocAnnotationsList404ResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocAnnotationsList404ResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocAnnotationsList404ResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocAnnotationsList404ResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocAnnotationsList404ResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocAnnotationsList404ResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocAnnotationsList404ResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocAnnotationsList404ResponseCode.STALE_BASE:
            return stale_base()
