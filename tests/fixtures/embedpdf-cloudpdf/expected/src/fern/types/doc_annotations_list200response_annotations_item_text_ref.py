

from __future__ import annotations

import typing

import pydantic
import typing_extensions
from ..core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ..core.serialization import FieldMetadata
from .doc_annotations_list200response_annotations_item_text_ref_index_page import (
    DocAnnotationsList200ResponseAnnotationsItemTextRefIndexPage,
)
from .doc_annotations_list200response_annotations_item_text_ref_index_revision import (
    DocAnnotationsList200ResponseAnnotationsItemTextRefIndexRevision,
)
from .doc_annotations_list200response_annotations_item_text_ref_nm_page import (
    DocAnnotationsList200ResponseAnnotationsItemTextRefNmPage,
)
from .doc_annotations_list200response_annotations_item_text_ref_object_number_page import (
    DocAnnotationsList200ResponseAnnotationsItemTextRefObjectNumberPage,
)


class DocAnnotationsList200ResponseAnnotationsItemTextRef_ObjectNumber(UniversalBaseModel):
    kind: typing.Literal["objectNumber"] = "objectNumber"
    page: DocAnnotationsList200ResponseAnnotationsItemTextRefObjectNumberPage
    annot_object_number: typing_extensions.Annotated[
        int, FieldMetadata(alias="annotObjectNumber"), pydantic.Field(alias="annotObjectNumber")
    ]

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


class DocAnnotationsList200ResponseAnnotationsItemTextRef_Nm(UniversalBaseModel):
    kind: typing.Literal["nm"] = "nm"
    page: DocAnnotationsList200ResponseAnnotationsItemTextRefNmPage
    nm: str

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


class DocAnnotationsList200ResponseAnnotationsItemTextRef_Index(UniversalBaseModel):
    kind: typing.Literal["index"] = "index"
    page: DocAnnotationsList200ResponseAnnotationsItemTextRefIndexPage
    index: int
    revision: DocAnnotationsList200ResponseAnnotationsItemTextRefIndexRevision

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow


DocAnnotationsList200ResponseAnnotationsItemTextRef = typing_extensions.Annotated[
    typing.Union[
        DocAnnotationsList200ResponseAnnotationsItemTextRef_ObjectNumber,
        DocAnnotationsList200ResponseAnnotationsItemTextRef_Nm,
        DocAnnotationsList200ResponseAnnotationsItemTextRef_Index,
    ],
    pydantic.Field(discriminator="kind"),
]
