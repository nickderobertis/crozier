...
class StackInstanceDriftStatus(str, enum.Enum):
    """
    ...
    def visit(
        ...
        if self is StackInstanceDriftStatus.DRIFTED:
            return drifted()
        if self is StackInstanceDriftStatus.IN_SYNC:
            return in_sync()
        if self is StackInstanceDriftStatus.UNKNOWN:
            return unknown()
        if self is StackInstanceDriftStatus.NOT_CHECKED:
            return not_checked()
