...
class StackStackStatus(str, enum.Enum):
    """
    ...
    def visit(
        ...
        if self is StackStackStatus.CREATE_IN_PROGRESS:
            return create_in_progress()
        if self is StackStackStatus.CREATE_FAILED:
            return create_failed()
        if self is StackStackStatus.CREATE_COMPLETE:
            return create_complete()
        if self is StackStackStatus.ROLLBACK_IN_PROGRESS:
            return rollback_in_progress()
        if self is StackStackStatus.ROLLBACK_FAILED:
            return rollback_failed()
        if self is StackStackStatus.ROLLBACK_COMPLETE:
            return rollback_complete()
        if self is StackStackStatus.DELETE_IN_PROGRESS:
            return delete_in_progress()
        if self is StackStackStatus.DELETE_FAILED:
            return delete_failed()
        if self is StackStackStatus.DELETE_COMPLETE:
            return delete_complete()
        if self is StackStackStatus.UPDATE_IN_PROGRESS:
            return update_in_progress()
        if self is StackStackStatus.UPDATE_COMPLETE_CLEANUP_IN_PROGRESS:
            return update_complete_cleanup_in_progress()
        if self is StackStackStatus.UPDATE_COMPLETE:
            return update_complete()
        if self is StackStackStatus.UPDATE_FAILED:
            return update_failed()
        if self is StackStackStatus.UPDATE_ROLLBACK_IN_PROGRESS:
            return update_rollback_in_progress()
        if self is StackStackStatus.UPDATE_ROLLBACK_FAILED:
            return update_rollback_failed()
        if self is StackStackStatus.UPDATE_ROLLBACK_COMPLETE_CLEANUP_IN_PROGRESS:
            return update_rollback_complete_cleanup_in_progress()
        if self is StackStackStatus.UPDATE_ROLLBACK_COMPLETE:
            return update_rollback_complete()
        if self is StackStackStatus.REVIEW_IN_PROGRESS:
            return review_in_progress()
        if self is StackStackStatus.IMPORT_IN_PROGRESS:
            return import_in_progress()
        if self is StackStackStatus.IMPORT_COMPLETE:
            return import_complete()
        if self is StackStackStatus.IMPORT_ROLLBACK_IN_PROGRESS:
            return import_rollback_in_progress()
        if self is StackStackStatus.IMPORT_ROLLBACK_FAILED:
            return import_rollback_failed()
        if self is StackStackStatus.IMPORT_ROLLBACK_COMPLETE:
