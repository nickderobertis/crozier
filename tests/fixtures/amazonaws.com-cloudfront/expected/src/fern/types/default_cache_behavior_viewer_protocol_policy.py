...
class DefaultCacheBehaviorViewerProtocolPolicy(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is DefaultCacheBehaviorViewerProtocolPolicy.ALLOW_ALL:
            return allow_all()
        if self is DefaultCacheBehaviorViewerProtocolPolicy.HTTPS_ONLY:
            return https_only()
        if self is DefaultCacheBehaviorViewerProtocolPolicy.REDIRECT_TO_HTTPS:
