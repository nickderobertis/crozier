...
class HookStatus(enum.StrEnum):
    ...
    def visit(
        ...
        if self is HookStatus.HOOK_IN_PROGRESS:
            return hook_in_progress()
        if self is HookStatus.HOOK_COMPLETE_SUCCEEDED:
            return hook_complete_succeeded()
        if self is HookStatus.HOOK_COMPLETE_FAILED:
            return hook_complete_failed()
        if self is HookStatus.HOOK_FAILED:
            return hook_failed()
