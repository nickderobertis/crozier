...
/// Whether a string a YAML document supplied as an example would have been
/// resolved to a *timestamp* rather than to a string, had it been written
/// unquoted. Fern's YAML parser follows YAML 1.1 here, so an unquoted
/// `2022-01-23T19:00:00.000Z` reaches its example generator as a date and is not
/// a value a plain `str` field can take — VTEX writes its `dateRange` example
/// that way and its golden falls back to the field-name placeholder, while the
/// same document written as JSON keeps the string (probed at 5.20.0 both ways).
/// The quoting itself does not survive deserialization, so this reads the value.
fn yaml_resolves_as_timestamp(value: &str) -> bool {
    let bytes = value.as_bytes();
    // `yyyy-mm-dd`, then optionally `T`/space and at least `hh:mm`.
    if bytes.len() < 10 {
        return false;
    }
...
/// the way Fern's class and field writers do so the value round-trips. Every line
/// (including blank ones) carries the block indent; the block has no trailing
/// newline.
fn render_docstring(doc: &str, indent: usize) -> String {
    render_doc_block(doc, indent, true)
}

/// The same block for an *alias* module docstring, which Fern writes verbatim.
/// Only the class and field writers escape: `dnd5eapi.co`'s `DC.success_type`
/// field carries `\\"none\\"` for a source `\"none\"`, while Listen Notes'
/// `TranscriptField` alias describes "the newline character \n" and its golden
/// keeps that one backslash.
fn render_alias_docstring(doc: &str, indent: usize) -> String {
    render_doc_block(doc, indent, false)
}

fn render_doc_block(doc: &str, indent: usize, escape: bool) -> String {
    let pad = " ".repeat(indent);
            ...
            for f in &form.fields {
                if let Some(content_type) = &f.form_content_type {
                    if f.form_json {
                        ...
                        let encoded = if is_json_like_media_type(content_type) {
    ...
    fn render_at(&self, indent: usize, column: usize) -> String {
        match self {
            ...
            Example::List(items) => {
                ...
                // and stays flat where its 90 UTF-8 bytes would have wrapped it.
                if !self.forces_multiline() && column + self.flat().chars().count() <= 80 {
                    return self.flat();
                }
                let pad = " ".repeat(indent);
                let inner_pad = " ".repeat(indent + 4);
            ...
            Example::ExplicitList(items) => {
                ...
                if !matrix && !self.forces_multiline() && column + self.flat().chars().count() <= 80
                {
                    return self.flat();
                }
                let pad = " ".repeat(indent);
            ...
            Example::Dict(pairs) | Example::DocDict(pairs) => {
                let wraps_for_length =
                    !self.forces_multiline() && column + self.flat().chars().count() > 80;
...
"#
    ...
    #[test]
    fn bytes_body_endpoint_is_not_readme_eligible() {
