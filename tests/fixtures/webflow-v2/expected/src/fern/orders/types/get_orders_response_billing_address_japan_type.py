...
class GetOrdersResponseBillingAddressJapanType(enum.StrEnum):
    """
    ...
    def visit(self, kana: typing.Callable[[], T_Result], kanji: typing.Callable[[], T_Result]) -> T_Result:
        if self is GetOrdersResponseBillingAddressJapanType.KANA:
            return kana()
        if self is GetOrdersResponseBillingAddressJapanType.KANJI:
            return kanji()
