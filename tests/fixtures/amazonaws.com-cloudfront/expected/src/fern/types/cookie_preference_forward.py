...
class CookiePreferenceForward(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is CookiePreferenceForward.NONE:
            return none()
        if self is CookiePreferenceForward.WHITELIST:
            return whitelist()
        if self is CookiePreferenceForward.ALL:
            return all_()
