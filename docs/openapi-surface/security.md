...
`golden` row: its witness was registered as [`CORPUS.md`](../../tests/fixtures/CORPUS.md)
row 96 and its Fern 5.20.0 golden byte-matches with no exclusions. The document
registered is **not** the one this table records the row on. `mosip/esignet` is
MPL-2.0, which is outside the Apache-2.0/MIT/BSD/CC0 set every other manifest row
carries, so the registration took the MIT-licensed fallback this table's own
`http-dpop` cell names for exactly that case — `blackadi/OAUTH2.0`
`server/src/routes/openapi.json` at `b6e4cfa1fb060ca5ca3e32185f4a5d88c27163e3`,
whose repository carries an MIT `LICENSE` at that ref, and which declares
`scheme: dpop` beside `bearer` and `basic` and passes `fern check` at exit 0.
Registering it moved the census from 126 registered sources to 127 and 109
golden-bearing to 110, and `securityScheme.scheme=dpop` from an absence to one
site. `http-hoba` becomes a `gap` whose settlement is `FIXTURE`, on this table's
...
`http-dpop` golden has now shown does not hold a paired witness out of the
corpus.
