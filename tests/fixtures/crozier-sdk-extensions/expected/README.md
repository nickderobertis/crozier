...
async def main() -> None:
    await client.widgets.watch_stream()
...
pager = client.catalog.widgets.list(...)
...
client = FernApi(...)
response = client.widgets.with_raw_response.watch_stream(...)
...
```python
client.widgets.watch_stream(..., request_options={
    "max_retries": 1
})
```
