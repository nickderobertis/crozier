...
class HandlerErrorCode(str, enum.Enum):
    ...
    def visit(
        ...
        if self is HandlerErrorCode.NOT_UPDATABLE:
            return not_updatable()
        if self is HandlerErrorCode.INVALID_REQUEST:
            return invalid_request()
        if self is HandlerErrorCode.ACCESS_DENIED:
            return access_denied()
        if self is HandlerErrorCode.INVALID_CREDENTIALS:
            return invalid_credentials()
        if self is HandlerErrorCode.ALREADY_EXISTS:
            return already_exists()
        if self is HandlerErrorCode.NOT_FOUND:
            return not_found()
        if self is HandlerErrorCode.RESOURCE_CONFLICT:
            return resource_conflict()
        if self is HandlerErrorCode.THROTTLING:
            return throttling()
        if self is HandlerErrorCode.SERVICE_LIMIT_EXCEEDED:
            return service_limit_exceeded()
        if self is HandlerErrorCode.NOT_STABILIZED:
            return not_stabilized()
        if self is HandlerErrorCode.GENERAL_SERVICE_EXCEPTION:
            return general_service_exception()
        if self is HandlerErrorCode.SERVICE_INTERNAL_ERROR:
            return service_internal_error()
        if self is HandlerErrorCode.NETWORK_FAILURE:
            return network_failure()
        if self is HandlerErrorCode.INTERNAL_FAILURE:
            return internal_failure()
        if self is HandlerErrorCode.INVALID_TYPE_CONFIGURATION:
            return invalid_type_configuration()
        if self is HandlerErrorCode.HANDLER_INTERNAL_FAILURE:
            return handler_internal_failure()
        if self is HandlerErrorCode.NON_COMPLIANT:
            return non_compliant()
        if self is HandlerErrorCode.UNKNOWN:
            return unknown()
        if self is HandlerErrorCode.UNSUPPORTED_TARGET:
