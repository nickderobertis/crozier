...
class PostHooksTrigger(enum.StrEnum):
    ...
    def visit(
        ...
        if self is PostHooksTrigger.ACTION_RUN_HAS_COMPLETED:
            return action_run_has_completed()
        if self is PostHooksTrigger.DOCUMENT_IS_CREATED:
            return document_is_created()
        if self is PostHooksTrigger.EMAIL_IS_RECEIVED:
            return email_is_received()
        if self is PostHooksTrigger.PREDICTION_IS_CREATED:
            return prediction_is_created()
        if self is PostHooksTrigger.VALIDATION_TASK_HAS_COMPLETED:
            return validation_task_has_completed()
        if self is PostHooksTrigger.VALIDATION_TASK_IS_CREATED:
