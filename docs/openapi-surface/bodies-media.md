...
| media-type-schema | both | Media Type Object.schema | golden | Census `mediaType.schema`: 10,343 declarations in every registered fixture except `sse-streaming` and `calorieninjas.com` (122 fixtures). The Schema Object and its keywords are classified in the [schemas region](schemas.md). | | | |
...
| encoding-object | both | Media Type Object.encoding | golden | Census `mediaType.encoding`: 50 declarations in `free5gc-namf-communication` (9) and `free5gc-pdu-session` (41). Ledger `encoding-object`: `implements (`contentType`) / discards (`headers`)`. | | | |
| media-type-concrete | both | Media Type Object content-map key | golden | Census object-model map-key walk: 10,360 declarations that name both a type and a subtype, in 122 fixtures — every registered source except `bungie.net` (which keys its content maps `*/*` alone) and `calorieninjas.com` (which declares no content map at all). `query-parameters-openapi` was a third until the free-map-key walk repair reached its one `application/json` key, which sits under an unquoted-integer status code (see the method notes). | | | |
...
| media-type-suffix-json | both | Media Type Object content-map key | golden | Census object-model map-key walk: 237 declarations in `anchore.io` (5), `apicurio.local-registry` (6), `box.com` (9), `github.com` (2), `redocly.com-museum` (3), `free5gc-pdu-session` (66), `free5gc-namf-communication` (142), and `openepcis-dpp-ready` (4). One of the `free5gc-pdu-session` declarations is the key `application/+json`, which also witnesses `media-type-malformed-key` below; the other 65, and all 142 of `free5gc-namf-communication`'s, are `application/problem+json` keys the free-map-key walk repair reached. | | | |
...
| xml-request | both | Request Body Object.content `application/xml` key | limitations | Census map-key walk: zero declarations across every registered source. Ledger `xml-request`: `discards`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
| xml-response | both | Response Object.content `application/xml` key | limitations | Census map-key walk: zero declarations across every registered source. Ledger `xml-response`: `discards`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
...
| media-type-range | both | Media Type Object content-map key | golden | Census object-model map-key walk for a `type/*` range other than `*/*`, re-run over the 168 registered sources for this change: **22** declarations in five golden-bearing sources — `sftpgo` (corpus row 134, ten over five distinct ranges: `application/*`, `text/*`, `image/*`, `audio/*` and `video/*`, each on both `POST /shares/{id}/{fileName}` and `POST /user/files/upload`, and the corpus's only range on a **request** body), `torrentarr` (corpus row 127, six: `…/artist/{artist_id}/thumbnail`, `…/movie/{id}/thumbnail` and `…/series/{id}/thumbnail`, each under both `/api` and `/web`), `audiobookshelf` (corpus row 136, three `image/*` responses on the `GET`, `POST` and `PATCH` of `/api/authors/{id}/image`, the `GET`'s beside `image/webp` and `image/jpeg` over the same schema), `api-openverse-org` (corpus row 124, two `image/*` responses) and `komga` (corpus row 130, one: the `default` response of `GET /api/v1/books/{bookId}/pages/{pageNumber}`) — every one over `{type: string, format: binary}`, against zero in every other registered source (the only other key holding a `*` is `eozilla`'s malformed `/*`, which the `media-type-malformed-key` row carries). The **seven**-in-two figure this cell carried predates rows 134 and 136 and missed row 124's two, and is corrected rather than extended here. Rows 134 and 136 are what pin the request side and the range-beside-concrete-types side that row 127 alone could not |  |  |  |
| media-type-multipart | both | Media Type Object content-map key | golden | Census object-model map-key walk for `multipart/*`: 76 declarations in `form-bodies` (1), `anchore.io` (1), `appwrite.io-client` (1), `appwrite.io-server` (2), `asana.com` (1), `atlassian.com-jira` (4), `box.com` (3), `discourse.local` (1), `microcks.local` (2), `appng-rest-api` (1), `free5gc-pdu-session` (41), `letta` (3), `free5gc-namf-communication` (9), and `livepeer-ai-runner` (6). | | | |
...
| encoding-content-type | both | Encoding Object.contentType | golden | Census `mediaType.encoding.contentType`: 122 declarations in `free5gc-namf-communication` (23) and `free5gc-pdu-session` (99). The ledger's `encoding-object` row counted 99 off the raw source for `free5gc-pdu-session`, against the 22 the census used to reach; the free-map-key walk repair closes that gap exactly, and the two measurements now agree. Ledger `encoding-object`: `implements (`contentType`) / discards (`headers`)`. | | | |
| encoding-headers | both | Encoding Object.headers | golden | Census `mediaType.encoding.headers`: 72 declarations in `free5gc-namf-communication` (14) and `free5gc-pdu-session` (58), agreeing with the ledger's own 58 for `free5gc-pdu-session` since the walk repair, as `encoding-content-type` above does. Ledger `encoding-object`: `implements (`contentType`) / discards (`headers`)`. Header Object behavior is classified in the [parameters region](parameters.md). | | | |
| encoding-style | both | Encoding Object.style | golden | Census `mediaType.encoding.style`: 122 declarations in `free5gc-namf-communication` (23) and `free5gc-pdu-session` (99), all `form`. | | | |
| encoding-explode | both | Encoding Object.explode | limitations | Census `mediaType.encoding.explode`: zero declarations across every registered source. Ledger `encoding-explode-or-allowReserved`: `refuses (multipart object `explode`) / ignores (list `explode`, `allowReserved`)`. **proof outstanding:** `differential` — the ledger cell rules on two features here: list `explode` `ignores`, so this row owes a probe and control isolating it, and the multipart object `explode` `refuses`, which owes a `refusal` record beside that pair; both are non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule) and neither proof is committed yet. | | | |
| encoding-allow-reserved | both | Encoding Object.allowReserved | limitations | Census `mediaType.encoding.allowReserved`: zero declarations across every registered source. Ledger `encoding-explode-or-allowReserved`: `refuses (multipart object `explode`) / ignores (list `explode`, `allowReserved`)`. **proof outstanding:** `differential` — the cell's `allowReserved` half reads `ignores`, which is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes a probe and control isolating it; the `refuses` half is `encoding-explode`'s object form, not this feature. The pair is not committed yet. | | | |
| response-status-literal | both | Responses Object status-code key | golden | Census object-model map-key walk: 15,930 literal status declarations in 121 fixtures, every registered source except `calorieninjas.com`, `reverb.com`, and `sigstore-rekor`. Five more were exceptions until the free-map-key walk repair — `free5gc-namf-communication` (176), `free5gc-pdu-session` (110), `worldcoin-signup-sequencer` (17), `kytos-sdntrace-cp` (4) and `query-parameters-openapi` (1) — each of which declares its status codes in the unquoted-integer YAML form the walk used to skip (see the method notes). | | | |
| range-1XX | both | Responses Object `1XX` key | limitations | Census map-key walk: zero declarations across every registered source. Ledger `range-1XX`: `discards + supply`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
| range-2XX | both | Responses Object `2XX` key | golden | Census map-key walk: eight declarations, all in `sigstore-rekor`. | | | |
| range-3XX | both | Responses Object `3XX` key | limitations | Census map-key walk: zero declarations across every registered source. Ledger `range-3XX`: `discards`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
| range-4XX | both | Responses Object `4XX` key | limitations | Census map-key walk: zero declarations across every registered source. Ledger `range-4XX`: `discards`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
| range-5XX | both | Responses Object `5XX` key | limitations | Census map-key walk: zero declarations across every registered source. Ledger `range-5XX`: `discards`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
...
| response-description | both | Response Object.description | golden | Census `response.description`: 11,410 declarations in every registered fixture (124 fixtures). The three that declared none — `query-parameters-openapi`, `worldcoin-signup-sequencer` and `kytos-sdntrace-cp` — write their Response Objects under unquoted-integer status codes, and the free-map-key walk repair reads them. | | | |
| response-headers | both | Response Object.headers | golden | Census `response.headers`: 704 declarations in `apideck.com-proxy`, `apideck.com-vault`, `box.com`, `bunq.com`, `canada-holidays.ca`, `electric-sql`, `eozilla`, `esgenterprise.com`, `free5gc-namf-communication`, `free5gc-pdu-session`, `github.com`, `groundhog-day.com`, `microcks.local`, `openbanking.org.uk-account-info-openapi`, `openepcis-dpp-ready`, `sigstore-rekor`, and `tamoss` (17 fixtures). | | | |
| response-content | both | Response Object.content | golden | Census `response.content`: 8,302 declarations in every registered fixture except `nested-core-imports`, `recursive-types`, `calorieninjas.com`, and `reverb.com` (120 fixtures). The other four former exceptions — `query-parameters-openapi`, `worldcoin-signup-sequencer`, `free5gc-namf-communication` and `kytos-sdntrace-cp` — declare a content map under an unquoted-integer status code, which the free-map-key walk repair reads. | | | |
...
| link-requestBody | both | Link Object.requestBody | limitations | Census `link.requestBody`: zero declarations across every registered source. Ledger `link-requestBody`: `discards + supply`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
| link-description | both | Link Object.description | limitations | Census `link.description`: zero declarations across every registered source. Ledger `link-description`: `discards`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
| link-server | both | Link Object.server | limitations | Census `link.server`: zero declarations across every registered source. Ledger `link-server`: `discards + supply`. **proof outstanding:** `absent-tree` — verdict `discards` is non-generation under [the amended rule](../openapi-surface-coverage.md#the-limitations-rows-under-the-amended-rule), so this row owes its Fern tree, declared in the probe manifest; it is not committed yet. | | | |
...
That last clause was load-bearing, and the defect it described is now fixed. The
walk used to skip a map key that is not a string *and the descent under it*, so a
status code written as an unquoted YAML integer — `200:` rather than `"200":` —
took the whole Response Object beneath it out of the census. Six registered
sources write their status codes that way: `free5gc-namf-communication`,
`free5gc-pdu-session`, `kytos-sdntrace-cp`, `marimo`, `query-parameters-openapi`
and `worldcoin-signup-sequencer` — one more than this paragraph used to name,
because `marimo` was not in the walk these cells were taken on. Every
Response-side count above was therefore a **floor** for those sources. The size
of the gap was visible in the `encoding-object` row of
...
instrument was repaired: `media-type-schema` 10,025 → 10,343, `media-type-concrete`
10,042 → 10,360, `media-type-suffix-json` 30 → 237, `media-type-multipart` 42 →
76, `encoding-object` 16 → 50, `encoding-content-type` and `encoding-style` 40 →
122 each, `encoding-headers` 24 → 72, `response-status-literal` 15,622 → 15,930,
`response-description` 11,102 → 11,410, `response-headers` 698 → 704 and
`response-content` 8,018 → 8,302. `marimo`'s own gains are not added, since it is
not in the walk these cells rest on. No row's *category* turned on any of it: the
repair moves counts, and takes no selector in this region from zero declarations
corpus-wide to any — what it does move is which fixtures a row names, and four
rows above shed exceptions because of it.
...
one, is `golden` on corpus row 127, and on rows 130, 134 and 136 beside it.
