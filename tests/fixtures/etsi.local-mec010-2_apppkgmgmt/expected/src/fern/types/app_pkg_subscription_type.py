...
class AppPkgSubscriptionType(str, enum.Enum):
    """
    ...
    def visit(
        ...
        if self is AppPkgSubscriptionType.APP_PACKAGE_ON_BOARDING:
            return app_package_on_boarding()
        if self is AppPkgSubscriptionType.APP_PACAKGE_OPERATION_CHANGE:
            return app_pacakge_operation_change()
        if self is AppPkgSubscriptionType.APP_PACKAGE_DELETION:
