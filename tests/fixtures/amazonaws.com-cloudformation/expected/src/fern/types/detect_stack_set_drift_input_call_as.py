...
class DetectStackSetDriftInputCallAs(str, enum.Enum):
    """
    ...
    def visit(self, self_: typing.Callable[[], T_Result], delegated_admin: typing.Callable[[], T_Result]) -> T_Result:
        if self is DetectStackSetDriftInputCallAs.SELF:
            return self_()
        if self is DetectStackSetDriftInputCallAs.DELEGATED_ADMIN:
