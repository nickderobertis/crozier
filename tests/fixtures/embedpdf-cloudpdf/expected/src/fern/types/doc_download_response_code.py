...
class DocDownloadResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocDownloadResponseCode.UNKNOWN:
            return unknown()
        if self is DocDownloadResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocDownloadResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocDownloadResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocDownloadResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocDownloadResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocDownloadResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocDownloadResponseCode.ABORTED:
            return aborted()
        if self is DocDownloadResponseCode.NETWORK:
            return network()
        if self is DocDownloadResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocDownloadResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocDownloadResponseCode.NOT_FOUND:
            return not_found()
        if self is DocDownloadResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocDownloadResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocDownloadResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocDownloadResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocDownloadResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocDownloadResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocDownloadResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocDownloadResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocDownloadResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocDownloadResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocDownloadResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocDownloadResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocDownloadResponseCode.STALE_BASE:
            return stale_base()
