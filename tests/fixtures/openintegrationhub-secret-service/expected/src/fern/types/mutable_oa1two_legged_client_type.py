...
class MutableOa1TwoLeggedClientType(enum.StrEnum):
    ...
    def visit(
        ...
        if self is MutableOa1TwoLeggedClientType.OA1TWO_LEGGED:
            return oa1two_legged()
        if self is MutableOa1TwoLeggedClientType.OA1THREE_LEGGED:
            return oa1three_legged()
        if self is MutableOa1TwoLeggedClientType.OA2AUTHORIZATION_CODE:
            return oa2authorization_code()
        if self is MutableOa1TwoLeggedClientType.SESSION_AUTH:
            return session_auth()
