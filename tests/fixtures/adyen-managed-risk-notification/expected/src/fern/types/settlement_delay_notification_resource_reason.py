...
class SettlementDelayNotificationResourceReason(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is SettlementDelayNotificationResourceReason.PAYMENT_PROCESSING_ENABLED:
            return payment_processing_enabled()
        if self is SettlementDelayNotificationResourceReason.SETTLEMENT_DELAY_CHANGED:
            return settlement_delay_changed()
        if self is SettlementDelayNotificationResourceReason.RISK_REVIEW_PERIOD_PASSED:
