...
`tests/fixtures/<name>/openapi.*` documents, and the 146 `link-ok` documents
...
- **`just surface-census`**, for the classifications and for criterion 4. The
  ...
  It reads **178** registered sources, of which
  **161** carry a committed golden. `document-paths`'s own evidence cells are
...
The witness searches' registrations add six sources — corpus rows 144 to 149,
`paloalto-cspm-alerts`, `paloalto-cspm-reports`, `paloalto-cspm-search-manager`,
`thrivecart`, `truefoundry-trueforge-5adde28` and `fergus` (32 vendored plus 146
`link-ok`) — and promote five `schemas` features to `golden`:
