...
class CacheBehaviorListItemViewerProtocolPolicy(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is CacheBehaviorListItemViewerProtocolPolicy.ALLOW_ALL:
            return allow_all()
        if self is CacheBehaviorListItemViewerProtocolPolicy.HTTPS_ONLY:
            return https_only()
        if self is CacheBehaviorListItemViewerProtocolPolicy.REDIRECT_TO_HTTPS:
