...
class ResourceStatus(enum.StrEnum):
    ...
    def visit(
        ...
        if self is ResourceStatus.CREATE_IN_PROGRESS:
            return create_in_progress()
        if self is ResourceStatus.CREATE_FAILED:
            return create_failed()
        if self is ResourceStatus.CREATE_COMPLETE:
            return create_complete()
        if self is ResourceStatus.DELETE_IN_PROGRESS:
            return delete_in_progress()
        if self is ResourceStatus.DELETE_FAILED:
            return delete_failed()
        if self is ResourceStatus.DELETE_COMPLETE:
            return delete_complete()
        if self is ResourceStatus.DELETE_SKIPPED:
            return delete_skipped()
        if self is ResourceStatus.UPDATE_IN_PROGRESS:
            return update_in_progress()
        if self is ResourceStatus.UPDATE_FAILED:
            return update_failed()
        if self is ResourceStatus.UPDATE_COMPLETE:
            return update_complete()
        if self is ResourceStatus.IMPORT_FAILED:
            return import_failed()
        if self is ResourceStatus.IMPORT_COMPLETE:
            return import_complete()
        if self is ResourceStatus.IMPORT_IN_PROGRESS:
            return import_in_progress()
        if self is ResourceStatus.IMPORT_ROLLBACK_IN_PROGRESS:
            return import_rollback_in_progress()
        if self is ResourceStatus.IMPORT_ROLLBACK_FAILED:
            return import_rollback_failed()
        if self is ResourceStatus.IMPORT_ROLLBACK_COMPLETE:
            return import_rollback_complete()
        if self is ResourceStatus.UPDATE_ROLLBACK_IN_PROGRESS:
            return update_rollback_in_progress()
        if self is ResourceStatus.UPDATE_ROLLBACK_COMPLETE:
            return update_rollback_complete()
        if self is ResourceStatus.UPDATE_ROLLBACK_FAILED:
            return update_rollback_failed()
        if self is ResourceStatus.ROLLBACK_IN_PROGRESS:
            return rollback_in_progress()
        if self is ResourceStatus.ROLLBACK_COMPLETE:
            return rollback_complete()
        if self is ResourceStatus.ROLLBACK_FAILED:
