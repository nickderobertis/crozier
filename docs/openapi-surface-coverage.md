...
`tests/fixtures/<name>/openapi.*` documents, and the 150 `link-ok` documents
...
- **`just surface-census`**, for the classifications and for criterion 4. The
  ...
  It reads **182** registered sources, of which
  **165** carry a committed golden. `document-paths`'s own evidence cells are
...
The witness searches' registrations add ten sources — corpus rows 144 to 153,
...
`timelyapp`, `nextgen` and `auto-agent-protocol` (32 vendored plus 150
