...
class DocSignaturesComplete409ResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocSignaturesComplete409ResponseCode.UNKNOWN:
            return unknown()
        if self is DocSignaturesComplete409ResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocSignaturesComplete409ResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocSignaturesComplete409ResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocSignaturesComplete409ResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocSignaturesComplete409ResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocSignaturesComplete409ResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocSignaturesComplete409ResponseCode.ABORTED:
            return aborted()
        if self is DocSignaturesComplete409ResponseCode.NETWORK:
            return network()
        if self is DocSignaturesComplete409ResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocSignaturesComplete409ResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocSignaturesComplete409ResponseCode.NOT_FOUND:
            return not_found()
        if self is DocSignaturesComplete409ResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocSignaturesComplete409ResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocSignaturesComplete409ResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocSignaturesComplete409ResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocSignaturesComplete409ResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocSignaturesComplete409ResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocSignaturesComplete409ResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocSignaturesComplete409ResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocSignaturesComplete409ResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocSignaturesComplete409ResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocSignaturesComplete409ResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocSignaturesComplete409ResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocSignaturesComplete409ResponseCode.STALE_BASE:
            return stale_base()
