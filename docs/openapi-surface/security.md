...
`conjur.local 1` — a DROPPED row, so `http-mutual` is a `gap` all the same. **Two
of the twenty have since moved**: on the 129-source walk
[`document-paths.md`](document-paths.md#snapshot-reconciliation) now pins,
`securityScheme.scheme=dpop` prints `blackadi-oauth2 1` and
`securityScheme.scheme=DPoP` prints `mosip-esignet 1` and
`openbankingproject-ch-kundenbeziehung 1` — corpus rows 96, 97 and 98, the three
witnesses the `http-dpop` entry is `golden` on. So the field takes six values across the registered sources —
`basic`, `bearer`, `dpop`, `DPoP`, `mutual`, `negotiate`, the DPoP scheme in both
its spellings — and nine of the fourteen registry names are `gap` rows rather
...
of 124 on the walk most of this file's counts rest on, and is 112 of 129 on the
...
four-dropped-schemes finding. Five of the region's six `PROBE` rows are IANA
`scheme` members today, and only two of those — `http-mutual` and `http-vapid` —
still rest on that finding: `http-dpop` is `golden`; `http-hoba`, `http-oauth`,
`http-scram-sha-1` and `http-scram-sha-256` are `FIXTURE` gaps; and
`http-concealed`, `http-gnap` and `http-privatetoken` stay `PROBE` on witness
supply. All eight are settled on the evidence below and recorded in **What this
table's first four rows became** and **What this table's second four rows
became** after the table. That
...
family out of the corpus; the finding still stands wherever a row's own cell
cites it, and `http-mutual` and `http-vapid` are the two `PROBE` rows that still
do. Of the four rows settled here, `http-oauth`, `http-scram-sha-1` and
