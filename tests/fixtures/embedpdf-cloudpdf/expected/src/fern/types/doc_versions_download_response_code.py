...
class DocVersionsDownloadResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocVersionsDownloadResponseCode.UNKNOWN:
            return unknown()
        if self is DocVersionsDownloadResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocVersionsDownloadResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocVersionsDownloadResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocVersionsDownloadResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocVersionsDownloadResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocVersionsDownloadResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocVersionsDownloadResponseCode.ABORTED:
            return aborted()
        if self is DocVersionsDownloadResponseCode.NETWORK:
            return network()
        if self is DocVersionsDownloadResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocVersionsDownloadResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocVersionsDownloadResponseCode.NOT_FOUND:
            return not_found()
        if self is DocVersionsDownloadResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocVersionsDownloadResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocVersionsDownloadResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocVersionsDownloadResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocVersionsDownloadResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocVersionsDownloadResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocVersionsDownloadResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocVersionsDownloadResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocVersionsDownloadResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocVersionsDownloadResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocVersionsDownloadResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocVersionsDownloadResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocVersionsDownloadResponseCode.STALE_BASE:
            return stale_base()
