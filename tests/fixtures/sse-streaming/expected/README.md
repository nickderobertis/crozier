...
client.messages.streammessages()
```
...
async def main() -> None:
    await client.messages.streammessages()
...
client = FernApi(...)
response = client.messages.with_raw_response.streammessages()
...
```python
client.messages.streammessages(request_options={
    "max_retries": 1
})
```
