...
class CacheBehaviorViewerProtocolPolicy(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is CacheBehaviorViewerProtocolPolicy.ALLOW_ALL:
            return allow_all()
        if self is CacheBehaviorViewerProtocolPolicy.HTTPS_ONLY:
            return https_only()
        if self is CacheBehaviorViewerProtocolPolicy.REDIRECT_TO_HTTPS:
