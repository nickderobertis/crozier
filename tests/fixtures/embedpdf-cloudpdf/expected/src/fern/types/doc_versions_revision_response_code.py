...
class DocVersionsRevisionResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocVersionsRevisionResponseCode.UNKNOWN:
            return unknown()
        if self is DocVersionsRevisionResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocVersionsRevisionResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocVersionsRevisionResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocVersionsRevisionResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocVersionsRevisionResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocVersionsRevisionResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocVersionsRevisionResponseCode.ABORTED:
            return aborted()
        if self is DocVersionsRevisionResponseCode.NETWORK:
            return network()
        if self is DocVersionsRevisionResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocVersionsRevisionResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocVersionsRevisionResponseCode.NOT_FOUND:
            return not_found()
        if self is DocVersionsRevisionResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocVersionsRevisionResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocVersionsRevisionResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocVersionsRevisionResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocVersionsRevisionResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocVersionsRevisionResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocVersionsRevisionResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocVersionsRevisionResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocVersionsRevisionResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocVersionsRevisionResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocVersionsRevisionResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocVersionsRevisionResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocVersionsRevisionResponseCode.STALE_BASE:
            return stale_base()
