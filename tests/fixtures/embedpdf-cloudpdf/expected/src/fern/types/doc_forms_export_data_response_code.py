...
class DocFormsExportDataResponseCode(enum.StrEnum):
    UNKNOWN = "Unknown"
    ...
    def visit(
        ...
        if self is DocFormsExportDataResponseCode.UNKNOWN:
            return unknown()
        if self is DocFormsExportDataResponseCode.INVALID_ARG:
            return invalid_arg()
        if self is DocFormsExportDataResponseCode.DOC_NOT_OPEN:
            return doc_not_open()
        if self is DocFormsExportDataResponseCode.DOC_OPEN_FAILED:
            return doc_open_failed()
        if self is DocFormsExportDataResponseCode.DOC_PASSWORD_REQUIRED:
            return doc_password_required()
        if self is DocFormsExportDataResponseCode.DOC_PASSWORD_INCORRECT:
            return doc_password_incorrect()
        if self is DocFormsExportDataResponseCode.SHARE_PASSWORD_REQUIRED:
            return share_password_required()
        if self is DocFormsExportDataResponseCode.ABORTED:
            return aborted()
        if self is DocFormsExportDataResponseCode.NETWORK:
            return network()
        if self is DocFormsExportDataResponseCode.UNAUTHENTICATED:
            return unauthenticated()
        if self is DocFormsExportDataResponseCode.FORBIDDEN:
            return forbidden()
        if self is DocFormsExportDataResponseCode.NOT_FOUND:
            return not_found()
        if self is DocFormsExportDataResponseCode.WIRE_FORMAT:
            return wire_format()
        if self is DocFormsExportDataResponseCode.RUNTIME_UNAVAILABLE:
            return runtime_unavailable()
        if self is DocFormsExportDataResponseCode.INVALID_REFERENCE:
            return invalid_reference()
        if self is DocFormsExportDataResponseCode.WEAK_ANNOTATION_SESSION_CONFLICT:
            return weak_annotation_session_conflict()
        if self is DocFormsExportDataResponseCode.LAYER_VERSION_CONFLICT:
            return layer_version_conflict()
        if self is DocFormsExportDataResponseCode.NOT_IMPLEMENTED:
            return not_implemented()
        if self is DocFormsExportDataResponseCode.MALFORMED_PDF:
            return malformed_pdf()
        if self is DocFormsExportDataResponseCode.SIGNING_PENDING:
            return signing_pending()
        if self is DocFormsExportDataResponseCode.SIGNING_EXPIRED:
            return signing_expired()
        if self is DocFormsExportDataResponseCode.SIGNING_VERSION_MISMATCH:
            return signing_version_mismatch()
        if self is DocFormsExportDataResponseCode.SIGNATURE_REFUSED:
            return signature_refused()
        if self is DocFormsExportDataResponseCode.PROTECTED_DOCUMENT:
            return protected_document()
        if self is DocFormsExportDataResponseCode.STALE_BASE:
            return stale_base()
