...
class TypeFiltersCategory(enum.StrEnum):
    ...
    def visit(
        ...
        if self is TypeFiltersCategory.REGISTERED:
            return registered()
        if self is TypeFiltersCategory.ACTIVATED:
            return activated()
        if self is TypeFiltersCategory.THIRD_PARTY:
            return third_party()
        if self is TypeFiltersCategory.AWS_TYPES:
            return aws_types()
