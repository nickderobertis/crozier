...
class AccountGateResultStatus(str, enum.Enum):
    """
    ...
    def visit(
        ...
        if self is AccountGateResultStatus.SUCCEEDED:
            return succeeded()
        if self is AccountGateResultStatus.FAILED:
            return failed()
        if self is AccountGateResultStatus.SKIPPED:
            return skipped()
