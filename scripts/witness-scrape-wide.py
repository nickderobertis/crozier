...
REGIONS = ('bodies-media', 'document-paths', 'oas31-extensions', 'parameters', 'schemas', 'security')
RANK_FIELDS = ('rank', 'artifact_sha256', 'artifact', 'keys', 'fern_evidence', 'comparison_evidence')
SLOT_FIELDS = ('slot', 'artifact_sha256', 'keys', 'disposition', 'evidence')
CANDIDATE_FIELDS = ('artifact', 'keys', 'redistribution', 'immutable publisher reference', 'Fern acceptance', 'retention', 'disposition', 'evidence')
...
ROWS = load('wide_rows', REPO / 'tests/surface_census_test.py').RankedBacklogTests.region_rows
...
def baseline(regions: Path, contract: Path) -> dict[str, dict]:
    ...
    for name in REGIONS:
        for row in ROWS((regions / f'{name}.md').read_text(encoding='utf-8')):
            if REDO.value(row[3]) != 'gap' or REDO.authoritative_details(row)[0] != 'search-incomplete':
                continue
...
def derive(args) -> None:
    ...
    for key, item in items.items():
        ...
        item['candidates'] = [
            {'artifact': REDO.value(row[0]), 'screens': row[2:6],
             'disposition': REDO.value(row[6]),
             'discarded': key in re.findall(r'`([^`]+)`', row[5].split('discarded keys:', 1)[1])
             if 'discarded keys:' in row[5] else False}
            for row in candidates if len(row) == 8 and key in re.findall(r'`([^`]+)`', row[1])
        ]
    args.report.mkdir(parents=True, exist_ok=True)
    write_json(args.report / 'baseline.json', {'schema_version': 1,
               'source_commit': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=REPO, encoding='utf-8').strip(),
               'keys': items})
    (args.report / 'keys.md').write_text('# Frozen derived baseline\n\n| key | selector |\n|---|---|\n' + ''.join(
        f'| `{key}` | `{item["selector"]}` |\n' for key, item in items.items()), encoding='utf-8')
...
def acquire(args) -> None:
    """Fetch each identity once; only a verified digest authorizes cached reuse."""
    inventory = read_inventory(args.inventory)
    LOCAL.contract_keys(args.contract)  # reject invalid selectors before acquisition
    ...
    for source in inventory['sources']:
        ...
        cached = args.cache / expected if expected else None
        try:
            ...
            if expected and expected != sha:
                row.update(status='digest-changed', diagnostic=f'expected {expected}, acquired {sha}; excluded from census')
                outcomes.append(row)
                continue
            (args.cache / sha).write_bytes(data)
            row['prior_relation'] = ('verified-already-scanned' if source.get('prior_sha256') == sha
                                     else 'changed-bytes' if source.get('prior_sha256') else 'not-proven-previously-scanned')
            ...
            try:
                ...
                if not isinstance(doc, dict) or not re.fullmatch(r'3\.[01]\.\d+(?:[-+].*)?', str(doc.get('openapi', ''))):
                    row.update(status='excluded', diagnostic='not an OpenAPI 3 document; no conversion performed')
                    path.unlink()
                else:
                    row.update(status='readable', document=path.name,
                               document_license=doc.get('info', {}).get('license'))
    ...
    for path in documents.iterdir():
        if path.name not in selected:
            path.unlink()
    output = {'schema_version': 1, 'inventory_sha256': digest(args.inventory.read_bytes()), 'sources': outcomes}
    write_json(args.output, output)
    run = subprocess.run(['just', 'witness-search-local-census', '--contract', str(args.contract),
                          '--workers', str(args.workers), '--documents', f'acquired={documents}'],
...
def validate(args) -> None:
    ...
    header = next((line for line in candidates.read_text(encoding='utf-8').splitlines() if line.startswith('| artifact')), '')
    ...
    if acquisition.is_file():
        aliases = {row['artifact']: row.get('sha256') for row in read_inventory(acquisition)['sources']}
    ...
    if [len(json.loads(ranks[k]['keys'])) for k in sorted(ranks)] != sorted((len(json.loads(row['keys'])) for row in ranks.values()), reverse=True):
        raise ValueError('ranking violates descending retained-key coverage')
    slots = set()
    claimed = set()
    registered_keys = set()
    ...
    if args.inventory:
        inventory = read_inventory(args.inventory)
        outcomes = read_inventory(root / 'acquisition.json')
        if outcomes.get('inventory_sha256') != digest(args.inventory.read_bytes()) or {r['artifact'] for r in inventory['sources']} != {r['artifact'] for r in outcomes['sources']}:
            raise ValueError('partial inventory accounting or inventory digest mismatch')
        for row in outcomes['sources']:
            if row.get('status') not in {'readable', 'unreadable', 'inaccessible', 'excluded', 'digest-changed'}:
...
def main() -> int:
    ...
    for command in ('derive', 'validate'):
        ...
        p.add_argument('--contract', type=Path, default=REPO / 'docs/openapi-surface/witness-search-redo/contract.md')
        if command == 'validate':
    ...
    try:
        {'derive': derive, 'acquire': acquire, 'validate': validate}[args.command](args)
