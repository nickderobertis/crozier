...
entries. Exactly eight specs genuinely pass Fern and are selected below; every
other unused entry failed Fern, so all 50 manifest rows are now accounted for
and there are no backups to invent. The selected goldens require Docker and
will be generated by the operator.
...
| `codesearch.debian.net` | compact conventional two-schema baseline | Selected — Fern golden generation pending |
