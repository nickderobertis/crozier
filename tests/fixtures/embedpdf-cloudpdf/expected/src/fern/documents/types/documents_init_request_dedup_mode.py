...
class DocumentsInitRequestDedupMode(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is DocumentsInitRequestDedupMode.ALWAYS_CREATE:
            return always_create()
        if self is DocumentsInitRequestDedupMode.REUSE_EXISTING:
