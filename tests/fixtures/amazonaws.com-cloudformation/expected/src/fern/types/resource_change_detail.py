...
class ResourceChangeDetail(UniversalBaseModel):
    ...
    target: typing_extensions.Annotated[
        typing.Optional[ResourceChangeDetailTarget],
        FieldMetadata(alias="Target"),
        pydantic.Field(
            alias="Target",
            description="A <code>ResourceTargetDefinition</code> structure that describes the field that CloudFormation will change and whether the resource will be recreated.",
        ),
    ] = None
    """
    ...
    evaluation: typing_extensions.Annotated[
        typing.Optional[ResourceChangeDetailEvaluation],
        FieldMetadata(alias="Evaluation"),
        pydantic.Field(
            alias="Evaluation",
            description="<p>Indicates whether CloudFormation can determine the target value, and whether the target value will change before you execute a change set.</p> <p>For <code>Static</code> evaluations, CloudFormation can determine that the target value will change, and its value. For example, if you directly modify the <code>InstanceType</code> property of an EC2 instance, CloudFormation knows that this property value will change, and its value, so this is a <code>Static</code> evaluation.</p> <p>For <code>Dynamic</code> evaluations, can't determine the target value because it depends on the result of an intrinsic function, such as a <code>Ref</code> or <code>Fn::GetAtt</code> intrinsic function, when the stack is updated. For example, if your template includes a reference to a resource that's conditionally recreated, the value of the reference (the physical ID of the resource) might change, depending on if the resource is recreated. If the resource is recreated, it will have a new physical ID, so all references to that resource will also be updated.</p>",
        ),
    ] = None
    """
    ...
    causing_entity: typing_extensions.Annotated[
        typing.Optional[str],
        FieldMetadata(alias="CausingEntity"),
        pydantic.Field(
            alias="CausingEntity",
