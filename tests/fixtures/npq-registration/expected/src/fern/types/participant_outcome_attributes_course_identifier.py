...
class ParticipantOutcomeAttributesCourseIdentifier(enum.StrEnum):
    """
    ...
    def visit(
        ...
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_SENIOR_LEADERSHIP:
            return npq_senior_leadership()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_HEADSHIP:
            return npq_headship()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_EXECUTIVE_LEADERSHIP:
            return npq_executive_leadership()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_EARLY_YEARS_LEADERSHIP:
            return npq_early_years_leadership()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_LEADING_TEACHING:
            return npq_leading_teaching()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_LEADING_BEHAVIOUR_CULTURE:
            return npq_leading_behaviour_culture()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_LEADING_TEACHING_DEVELOPMENT:
            return npq_leading_teaching_development()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_LEADING_LITERACY:
            return npq_leading_literacy()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_LEADING_PRIMARY_MATHEMATICS:
            return npq_leading_primary_mathematics()
        if self is ParticipantOutcomeAttributesCourseIdentifier.NPQ_SENCO:
            return npq_senco()
