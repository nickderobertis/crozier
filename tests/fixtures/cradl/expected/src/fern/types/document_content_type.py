...
class DocumentContentType(enum.StrEnum):
    ...
    def visit(
        ...
        if self is DocumentContentType.APPLICATION_PDF:
            return application_pdf()
        if self is DocumentContentType.IMAGE_JPEG:
            return image_jpeg()
        if self is DocumentContentType.IMAGE_PNG:
            return image_png()
        if self is DocumentContentType.IMAGE_TIFF:
            return image_tiff()
        if self is DocumentContentType.IMAGE_WEBP:
            return image_webp()
        if self is DocumentContentType.MESSAGE_RFC822:
