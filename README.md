...
Bootstrap enables the repository hooks and installs `sccache`. New worktrees get
an ignored, per-worktree Cargo config automatically: Cargo targets remain local
to avoid cross-worktree locking, while compiler results are shared under
`$ORCHESTRATOR_CACHE_DIR/sccache` (or the repository's common Git directory when
that variable is unset). If `sccache` is unavailable, the hook does nothing.
