...
class ListAgentsRequestIncludeItem(enum.StrEnum):
    ...
    def visit(
        ...
        if self is ListAgentsRequestIncludeItem.AGENT_BLOCKS:
            return agent_blocks()
        if self is ListAgentsRequestIncludeItem.AGENT_IDENTITIES:
            return agent_identities()
        if self is ListAgentsRequestIncludeItem.AGENT_MANAGED_GROUP:
            return agent_managed_group()
        if self is ListAgentsRequestIncludeItem.AGENT_PENDING_APPROVAL:
            return agent_pending_approval()
        if self is ListAgentsRequestIncludeItem.AGENT_SECRETS:
            return agent_secrets()
        if self is ListAgentsRequestIncludeItem.AGENT_SOURCES:
            return agent_sources()
        if self is ListAgentsRequestIncludeItem.AGENT_TAGS:
            return agent_tags()
        if self is ListAgentsRequestIncludeItem.AGENT_TOOLS:
            return agent_tools()
